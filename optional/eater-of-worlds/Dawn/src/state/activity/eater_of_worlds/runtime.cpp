    ...
    std::call_once(once, [] {
        ...
        try {
            ...
            if (!size || size >= path.size()) {
                ...
                document = coo::script::MissionDocument::read(
                    std::filesystem::path(path.data()).parent_path() / L"Dawn" / L"scripts"
        ...
        if (document) {
            std::snprintf(line.data(), line.size(),
                "ev=coo_script mission=eater_of_worlds result=loaded format=lua fnv1a64=%016llX path=Dawn/scripts/eater_of_worlds.lua reload=next_process",
