...
This is a read-only extractor.  It reads the generated Dawn build-data cache,
...
def cache_domains(cache: bytes) -> tuple[list[int], list[int]]:
    if cache[:8] != b"DAWNB" or u32(cache, 8) != CACHE_VERSION:
        raise ValueError("expected the version-54 Dawn build-data cache")
