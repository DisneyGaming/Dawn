...
1. Generic creation wrapper `0x16EE180` calls allocator `0x170F190`. This wrapper handles several types, despite an older Dawn diagnostic naming its return site after `player_broadcast`.
...
7. The replenishment call `0x4F88C0` sends message type 20. Dawn already parses this request and grants free IDs through its normal transactional ownership path. Its join path offers the available client share of the 8,192 IDs; current source does not contain a server-wide 150-ID hard cap. The available client cache and the server's full lease space are different quantities.

Relevant source:

- `Dawn/src/server/bap/encrypted/activity_message/activity_message_route.cpp`: `prepare_join` and `prepare_grant`.
- `Dawn/src/state/activity/entity_slots/transactions/activity_entity_slot_prepare.cpp`: chooses free IDs while preserving the server reserve and activity ownership.
- `Dawn/src/middleware/bap/activity_message/activity_entity_slot_request_parser.cpp`: type-20 requested count.
- `Dawn/src/client/hooks/bootflow/player_broadcast_create_probe.cpp`: existing layout references. This legacy diagnostic also changes allocation state and is not an appropriate read-only probe to enable.
