...
The policy applies only while Dawn has an active `queued` or `preparing` launch for the exact
...
call still sends native type 20, the Dawn server still selects actual free IDs through
...
revision rejection. The Release test passes. The full Release Dawn build also passes with zero
...
- `Dawn/src/client/hooks/bootflow/eater_entity_id_startup_policy.h`: pure route/profile policy
  and atomic temporary-profile guard.
- `Dawn/src/client/hooks/bootflow/eater_entity_id_startup.cpp`: exact-build qualification,
  detours, diagnostics, and safe lifecycle.
- `Dawn/src/client/hooks/bootflow/internal.h`: lifecycle declarations.
- `Dawn/src/client/hooks/bootflow/bootflow_hook_lifecycle.cpp`: installation, quiescence, and
  protected teardown integration.
- `Dawn/unit/eater_entity_id_startup_tests.cpp`: policy and real lease-transaction regressions.
- `Dawn/unit/eater_entity_id_startup_tests.vcxproj`: isolated Release test target.
