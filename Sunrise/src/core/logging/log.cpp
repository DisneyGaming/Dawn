...
/** Formats and emits one bounded record while the caller selects threshold admission. */
void write_record(Channel channel,
                  Level level,
                  std::string_view event,
                  bool applyThreshold) noexcept {
    ...
    if (!g_log.initialized || (applyThreshold && !enabled(channel, level))) {
...
void write_startup_record(Channel channel, std::string_view event) noexcept {
    write_record(channel, Level::info, event, false);
}
