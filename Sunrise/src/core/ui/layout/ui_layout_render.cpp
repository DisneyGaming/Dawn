...
void draw_content(const navigation::Selection& selected) noexcept {
    ...
    if (selected.descriptor.stable_id() != "client.mission_launch") {
...
bool render(bool visible) noexcept {
    ...
    const float width = (std::min)(1020.0F * dpi, viewport->Size.x - 32.0F * dpi);
    const float height = (std::min)(800.0F * dpi, viewport->Size.y - 32.0F * dpi);
    ...
    if (ImGui::Begin("Dawn", nullptr, flags)) {
        ...
        const float creditWidth = ImGui::CalcTextSize(credits::kBadgeLabel).x + ImGui::GetStyle().FramePadding.x * 2.0F;
        if (ImGui::GetContentRegionAvail().x > creditWidth + 240.0F * dpi) {
            ImGui::SameLine(ImGui::GetWindowWidth() - ImGui::GetStyle().WindowPadding.x - creditWidth);
        }
        if (ImGui::SmallButton(credits::kBadgeLabel)) { credits::request_open(); }
    }
    ImGui::End();
    ImGui::PopStyleVar();
    return true;
