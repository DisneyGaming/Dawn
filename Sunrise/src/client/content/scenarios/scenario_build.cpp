...
/** Resets the pass without materializing the multi-megabyte static group bank on the stack. */
void reset_roster(RosterStorage& roster) noexcept {
    for (ObjectMemo& memo : roster.memo) {
        memo = {};
        memo.group = kNotARosterGroup;
    }
    roster.groupCount = 0;
    roster.slotCount = 0;
    roster.slotsOverflowed = false;
    roster.unresolvedGroups = 0;
    roster.authoredOverflows = 0;
    roster.catalogOverflows = 0;
    roster.ordinaryOverflows = 0;
    roster.authoredReadFailures = 0;
    roster.keyConflicts = 0;
    roster.cursor = 0;
    roster.reads = 0;
}
...
void report(const Storage& storage,
    ...
    const int written =
        std::snprintf(line.data(),
                      ...
                      "groups=%zu dropped=%zu ordinary_overflow=%zu authored_overflow=%zu "
                      "catalog_overflow=%zu authored_read_failure=%zu key_conflict=%zu "
                      "rostered=%zu result=%s",
                      ...
                      storage.roster.ordinaryOverflows,
                      storage.roster.authoredOverflows,
                      storage.roster.catalogOverflows,
                      storage.roster.authoredReadFailures,
                      storage.roster.keyConflicts,
                      rostered,
                      result);
    if (written > 0) {
        const bool overflowed = storage.roster.ordinaryOverflows != 0
                                || storage.roster.authoredOverflows != 0
                                || storage.roster.catalogOverflows != 0
                                || storage.roster.authoredReadFailures != 0
                                || storage.roster.keyConflicts != 0;
        core::log::write(core::log::Channel::state,
                         kept != 0 && !overflowed ? core::log::Level::info
                                                  : core::log::Level::warn,
...
[[nodiscard]] bool walk_rosters(const packages::reader::Source& source,
    ...
    (void)probe_towerfall_cue_sources(
        source,
        scratch,
        rows,
        std::span(storage.roster.groups).first(storage.roster.groupCount));
