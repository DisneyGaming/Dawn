...
bool build() noexcept {
    ...
    if (domainsReady && homecomingDumped && omegaPropertyScanAttempted
        && state::build_data::activities::ready() && state::build_data::vendor_catalog_ready()) {
        return true;
    }
    ...
    // storage. Both are independent of the item table, so a failure here leaves it alone.
    {
        ...
        // This research sweep reads more than 1,700 Omega tags through the same package scratch.
        // Keep the cache-building path identical to the last successful roster census; diagnostics
        // may run only after every package-backed runtime domain has already been published.
        if (domainsReady && !omegaPropertyScanAttempted) {
            omegaPropertyScanAttempted = true;
            (void)content::activity::scan_omega_behavior_properties(packageSource,
                                                                    storage.scratch);
        }
    }
    if (domainsReady) {
