...
#include <atomic>
#include <cstdio>
namespace sunrise::client::hooks::bootflow::deadly_trial_revival {
namespace {
...
__declspec(noinline) std::uintptr_t __fastcall tick(void* component,void* output) noexcept {
    ...
    if(scope.accepts_side_effects()) { update(reinterpret_cast<std::uintptr_t>(component));deep_scan::update(reinterpret_cast<std::uintptr_t>(component));hijacked_scan::update(reinterpret_cast<std::uintptr_t>(component)); }
    return result;
}
