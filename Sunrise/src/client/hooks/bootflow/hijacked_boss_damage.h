...
#include "../../../state/activity/hijacked/boss_damage.h"
namespace sunrise::client::hooks::bootflow::hijacked_boss_damage {
...
// exact Entangled Mind body definition afresh at the existing damage boundary.
template<class Read> bool sample(Read& read,std::uintptr_t image,std::uintptr_t context,
    ...
    if(!request.owner.valid() || !request.enemy.valid() || request.enemy.run!=request.owner.run
        || request.enemy.generation!=request.owner.value || request.enemy.registry!=mission::kBoss.registry
        || request.enemy.source!=mission::kBoss.slot || request.stage>2) {return false;}
    ...
    if(!read.value(context,definition) || !read.value(context+8,health) || !read.copy(health,header)
        || mission::boss_damage::get<std::uint32_t>(header,0)!=0x815B5AA2U
        || mission::boss_damage::get<std::uint32_t>(header,4)!=0x80804B8AU
        || mission::boss_damage::get<std::int64_t>(header,8)!=0x1338
        || !read.resolve(0x815B5AA2U,base) || definition!=base+0x1338) {return false;}
    ...
    if(region!=base+0x1A30 || !read.copy(region,body)
        || mission::boss_damage::get<std::uint32_t>(body,0)!=0x815B5AA2U
        || mission::boss_damage::get<std::uint32_t>(body,4)!=0x80804BABU
        || mission::boss_damage::get<std::int64_t>(body,8)!=0x10F0
