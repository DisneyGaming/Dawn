...
void observe_hijacked_object(void* raw) noexcept {
    namespace ds=state::activity::hijacked;namespace gn=gateway_native;
    ...
    for(std::uint8_t i=0;i<std::size(ds::kPlates);++i) {if(binding->asset!=ds::kPlates[i].source) {continue;}
    ...
    for(std::uint8_t i=0;i<std::size(ds::kScans);++i) {if(binding->asset!=ds::kScans[i].source) {continue;}
