#pragma once
#include "../../../server/runtime/activity/public_event_engagement_bridge.h"
#include "adventure_cue_native_identity.h"

namespace sunrise::client::hooks::bootflow::public_event_engagement_capture {
namespace bridge=server::runtime::activity::public_event::engagement_bridge;
...
[[nodiscard]] Result begin(const bridge::Binding& binding,std::uintptr_t component,std::uintptr_t packet,
    ...
    if(!binding.epoch || !feedback::valid(binding.ticket)
        || (binding.ticket.request.collection!=feedback::wire::Collection::activePlayers && binding.ticket.request.collection!=feedback::wire::Collection::none))return Result::binding;
    ...
    if(feedback::field<std::uint32_t>(captured.prior,0)!=ticket.definition
        || feedback::field<std::uint32_t>(captured.prior,4)!=0x808094EF
    ...
    if(feedback::field<std::uint32_t>(definition,0x30)!=ticket.request.registry
        || feedback::field<std::uint8_t>(definition,0x34)!=70
        || feedback::field<std::uint16_t>(definition,0x36)!=ticket.request.slot
        || feedback::field<std::uint32_t>(definition,0x38)!=ticket.request.scope
        || feedback::field<std::uint32_t>(definition,0x48)!=0x808094F1)return Result::definition;
    ...
    if(feedback::field<std::uint32_t>(authority,0)!=ticket.request.registry
        || feedback::field<std::uint8_t>(authority,4)!=70
        || feedback::field<std::uint16_t>(authority,6)!=ticket.request.slot
        || feedback::field<std::uint32_t>(authority,0xC)!=0x808094F1
        || feedback::field<std::uint8_t>(authority,0x18)!=0
        || feedback::field<std::uint32_t>(authority,0x68)!=ticket.request.scope)return Result::authority;
...
// Called only after the unchanged original9F1820 callback. Two stable snapshots
...
[[nodiscard]] bool finish(const Context& context,std::uintptr_t component,std::uint64_t sequence,
    Reader&& read,Resolver&& resolve,Self&& self,feedback::Observation& out) noexcept {
    out={};
    ...
    const feedback::Capture capture{context.binding.ticket,context.self.source,sequence,feedback::kProducerRva,
        context.prior,after,incoming,context.authorityObject,feedback::wire::IdentifierEncoding::unknown,true};
