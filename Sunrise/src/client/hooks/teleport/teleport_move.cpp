...
/** One queued absolute move; the flag is released after the target is written. */
std::atomic_bool g_absolutePending{false};
std::array<float, kVectorLanes> g_absoluteTarget{};
/** One queued move to the nearest point of a named spawn set; zero means none. */
std::atomic_uint32_t g_hopSetHash{0};
/** Frames the camera hook keeps writing the requested view direction; zero means idle. */
std::atomic_uint32_t g_faceFrames{0};
std::array<float, kVectorLanes> g_faceTarget{};
 ...
 */
[[nodiscard]] inline bool read_at_span(const std::byte* address, void* out,
                                       std::size_t size) noexcept {
    if (address == nullptr) {
        return false;
    }
    SIZE_T read = 0;
    return ReadProcessMemory(GetCurrentProcess(), address, out, size, &read) != FALSE
           && read == size;
}
...
/**
 * Places the player's body at the queued absolute position.
 * @param component Physics component driving the player.
 * @return True when the body was found and its position was written.
 */
[[nodiscard]] bool perform_hop(std::byte* component, std::uint32_t setHash) noexcept {
    ...
    std::array<float, kVectorLanes> before{};
    if (!read_at(body + kBodyPositionX, before)) {
        report_skip("hop_body");
        return false;
    }
    if (setHash != 0) {
        // Resolve here, where the body position is certain: the nearest point of the set is
        // the side of the shared tunnel the player is standing next to.
        state::build_data::spawn_sets::Point point{};
        float distance = 0.0F;
        if (!state::build_data::find_nearest_spawn_point_of_set(
                setHash, before, point, distance)) {
            std::array<char, 96> miss{};
            const int missLength = std::snprintf(miss.data(), miss.size(),
                                                 "ev=teleport stage=hop result=no_set "
                                                 "set=0x%08X",
                                                 setHash);
            if (missLength > 0) {
                core::log::write(core::log::Channel::client, core::log::Level::warn,
                                 {miss.data(), static_cast<std::size_t>(missLength)});
            }
            return false;
        }
        g_absoluteTarget = point.position;
    }
    set_vertical_velocity(body, 0.0F);
    if (!write_vector(body + kBodyPositionX, g_absoluteTarget)) {
        report_skip("hop_body");
        return false;
    }
    ...
    const int written = std::snprintf(line.data(),
                                      line.size(),
                                      "ev=teleport stage=hop result=ok "
                                      "from=%.1f,%.1f,%.1f to=%.1f,%.1f,%.1f",
                                      static_cast<double>(before[0]),
                                      static_cast<double>(before[1]),
                                      static_cast<double>(before[2]),
                                      static_cast<double>(g_absoluteTarget[0]),
                                      static_cast<double>(g_absoluteTarget[1]),
                                      static_cast<double>(g_absoluteTarget[2]));
    if (written > 0) {
        core::log::write(core::log::Channel::client,
                         core::log::Level::info,
                         {line.data(), static_cast<std::size_t>(written)});
...
void capture_forward(std::uint32_t playerIndex) noexcept {
    ...
    // Pending view-direction request: the camera state stores the aim as forward-vector
    // copies (block +76, +1468, +2456; calibration found no yaw scalar, only the +1480 pitch),
    // so the view is turned by writing the copies for a few frames while the controller
    // integrates mouse deltas into them. Used by the Omega gate hop to face down the tunnel.
    std::uint32_t faceFrames = g_faceFrames.load(std::memory_order_acquire);
    if (faceFrames != 0) {
        g_faceFrames.store(faceFrames - 1, std::memory_order_release);
        std::byte* const block = camera + kCameraBlockStride * playerIndex;
        const std::array<float, kVectorLanes> target = g_faceTarget;
        (void)write_vector(block + 76, target);
        (void)write_vector(block + 1468, target);
        (void)write_vector(block + 2456, target);
        const float levelPitch = 0.0F;
        SIZE_T pitchWritten = 0;
        (void)WriteProcessMemory(GetCurrentProcess(), block + 1480, &levelPitch,
                                 sizeof levelPitch, &pitchWritten);
        if (faceFrames == 1) {
            core::log::write(core::log::Channel::client, core::log::Level::info,
                             "ev=teleport stage=face result=done");
        }
    }
}
...
void apply_pending(void* component) noexcept {
    ...
    if (!requested && !hopPending
        && g_playerComponent.load(std::memory_order_relaxed) != nullptr) {
        return;
    }
    ...
    if (hopPending) {
        g_absolutePending.store(false, std::memory_order_release);
        const std::uint32_t setHash = g_hopSetHash.exchange(0, std::memory_order_acq_rel);
        (void)perform_hop(physics, setHash);
    }
...
bool read_local_player_entity(void* component,std::uint32_t& entity) noexcept {
    entity=kInvalidHandle;
    if(component==nullptr || g_controlledHandle==nullptr) { return false; }
    std::uint32_t before=kInvalidHandle,owner=kInvalidHandle,after=kInvalidHandle;
    g_controlledHandle(&before);
    // Original4678B0 keeps a full owner at+2C, masking only when indexing.
    // Original4B2260 returns the complete controlled world handle. Retain salt;
    // a recycled low13 index is insufficient for an encounter arrival receipt.
    if(before==kInvalidHandle
...
void request_face_direction(const Vector& direction) noexcept {
    g_faceTarget = direction;
    g_faceFrames.store(45, std::memory_order_release);
}
