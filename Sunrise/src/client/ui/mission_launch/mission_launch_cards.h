...
[[nodiscard]] inline float card_scale() noexcept { return ImGui::GetFontSize() / 16.0F; }
...
[[nodiscard]] inline bool draw_icon(state::build_data::activities::Icon icon, ImVec2 origin, float extent) noexcept {
    ...
    ImGui::GetWindowDrawList()->AddImage(texture, {x, y}, {x + image.width, y + image.height}, {0, 0}, {1, 1},
...
[[nodiscard]] inline bool activity_card(const char* title, const char* subtitle,
    const char* footer, state::build_data::activities::Icon icon, bool selected, bool available = true,
    const char* artworkFallback = "Artwork unavailable") noexcept {
    const float scale = card_scale();
    const float width = (std::max)(1.0F, ImGui::GetContentRegionAvail().x);
    const bool clicked = ImGui::InvisibleButton("##activity_card", {width, std::ceil(188.0F * scale)}, ImGuiButtonFlags_EnableNav);
    const auto min = ImGui::GetItemRectMin(), max = ImGui::GetItemRectMax();
    auto* draw = ImGui::GetWindowDrawList();
    const bool hot = ImGui::IsItemHovered() || ImGui::IsItemFocused();
    draw->AddRectFilled(min, max, ImGui::GetColorU32(hot ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 7.0F * scale);
    draw->AddRect(min, max, ImGui::GetColorU32(hot || selected ? ImGuiCol_CheckMark : ImGuiCol_Border), 7.0F * scale,
        0, hot || selected ? 2.0F : 1.0F);
    ...
    if (!draw_icon(icon, {min.x + inset, min.y + 12.0F * scale}, 66.0F * scale)) {
        draw->AddText({min.x + inset, min.y + 35.0F * scale}, ImGui::GetColorU32(ImGuiCol_TextDisabled), artworkFallback);
    }
    ...
    draw->AddText(ImGui::GetFont(), ImGui::GetFontSize(), textMin,
        ImGui::GetColorU32(available ? ImGuiCol_Text : ImGuiCol_TextDisabled), title, nullptr,
        (std::max)(1.0F, textMax.x - textMin.x));
    draw->PopClipRect();
    draw->AddText({textMin.x, min.y + 137.0F * scale}, ImGui::GetColorU32(ImGuiCol_TextDisabled), subtitle);
    draw->AddText({textMin.x, min.y + 157.0F * scale}, ImGui::GetColorU32(available ? ImGuiCol_TextDisabled : ImGuiCol_PlotHistogram), footer);
    draw->PopClipRect();
    return clicked;
}
} // namespace sunrise::client::ui::mission_launch
