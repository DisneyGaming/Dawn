...
/** Removes one row of a bounded override list. The caller holds the state lock. */
template <typename Row, std::size_t Capacity>
void clear_row(std::array<Row, Capacity>& rows,
               std::size_t& count,
...
/** Records a future refetch request without invoking a client hook. */
void request_client_refetch() noexcept {
    g_refetchRequested.store(true, std::memory_order_release);
}

/** @return True once per recorded request. */
