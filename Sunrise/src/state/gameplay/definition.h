...
/** Fragments waiting to be written into outgoing packets.
...
struct OutboundQueue {
    ...
    /** Prefix of the queue carried by that packet. The retail queue normally sends one. */
...
struct PeerLink {
    ...
    /**
     * True after a packet arrives while the link is already connected. The packet that carries
     * the peer's reliable establishment changes the stage, but application messages queued in
     * that same exchange are acknowledged before the peer will dispatch them.
     */
