...
std::int32_t leaf(bool value,std::uint16_t slot,const AccountState& account,
    std::size_t ci,const Family5State& overrides) noexcept {
    ...
    const auto& table=unlocks::get();
    if(value) {
        if(b.bank==0 && b.row<table.objectiveValues.size()) {return table.objectiveValues[b.row];}
        if(b.bank==1 && b.row<table.characterObjectValues.size()) {return table.characterObjectValues[b.row];}
    } else {
        ...
        if(b.bank==2 && b.row<table.characterObjectFlags.size()) {return table.characterObjectFlags[b.row]==2;}
        if(b.bank==4 && b.row<table.characterFlags.size()) {return table.characterFlags[b.row]==2;}
    }
    return 0;
}
bool evaluate(std::span<const catalog::Instruction> program,const AccountState& a,std::size_t ci,
    const Family5State& overrides,std::int32_t& result,unsigned depth) noexcept {
    ...
    for(const auto i:program) {
        if(i.opcode==1 || i.opcode==10 || i.opcode==11 || i.opcode==12) {
            ...
            if(i.opcode==1 || i.opcode==10) {value=static_cast<std::uint32_t>(leaf(i.opcode==10,static_cast<std::uint16_t>(i.operand),a,ci,overrides));}
            if(i.opcode==12) {
                catalog::Expression pooled;std::int32_t v{};
                if(!catalog::pooled(static_cast<std::uint16_t>(i.operand),pooled) || !evaluate(pooled,a,ci,overrides,v,depth+1)) {return false;}
...
bool condition(std::span<const catalog::Instruction> p,const AccountState& a,std::size_t ci,
    const Family5State& f,std::int32_t& out) noexcept {out=0;return evaluate(p,a,ci,f,out,0);}
}
