...
bool commit(PendingAllocation& allocation) noexcept {
    ...
    if (!prepared.prepared || prepared.sessionId == kAbsentSessionId
        ...
        || prepared.instanceKey.sessionId != prepared.sessionId
        || prepared.instanceKey.incarnation.value != kFirstGeneration
    ...
    const std::size_t target = transactions::select_target(state, prepared.replaces);
    if (target == kInvalidSessionSlot || target != prepared.targetSlot) {
        ReleaseSRWLockExclusive(&runtime::storage::g_stateLock);
        return false;
    }

    SessionRecord& record = state.sessions[target];
    // A full reset restores each field's unset value before the fresh lifecycle is published.
    record = {};
    ++state.stateRevision;
