...
/** The music choice, beside it: one token from `kMusicTokens`, `#` comments. */
...
/**
 * What the event configuration writes above the keys, so a hand editor knows the file's meaning.
 */
constexpr char kHeader[] =
    "# Tower events. Every key listed here is WITHHELD from the roster, so the event it carries is "
    "not shown.\n"
    "# Managed by event configuration. Hand edits and the presets in event_presets work the same "
    "way.\n"
...
/** @return True when an artifact path resolves to an existing filesystem entry. */
[[nodiscard]] bool artifact_exists(std::wstring_view relative) noexcept {
    core::path::Buffer file{};
    return core::path::artifact_file(relative, file)
           && GetFileAttributesW(file.chars.data()) != INVALID_FILE_ATTRIBUTES;
}
...
/** Reads the file into one set. A missing file starts with only Festival of the Lost shown. */
void read_file(KeySet& keys) noexcept {
    ...
    if (!core::path::read_artifact_text(kFileName, text)) {
        if (!artifact_exists(kFileName)) {
            keys = kDefaultWithheldKeys;
        }
        return;
    }
...
void apply_flags(const KeySet& pending, Music music) noexcept {
    ...
    for (const EventFlag& entry : kEventFlags) {
        const bool live = flag_live(pending, music, entry);
        if (live) {
    ...
    // Authored Tower graph81327CF7: node4DE18C80 (classified activity255) and
    // node0889C1A0 both occupy the Festival map location and require12255.
    // Withhold that overlapping placeholder during FoTL; restore its underlying
    // policy when the event ends. Do not redirect activity255 or patch the map.
    if (!pending.contains(0x7C6DE64FU)) {
        set += investment::set_flag_override(12255,0) ? 1 : 0;
    } else {
        investment::clear_flag_override(12255);
        ++cleared;
    }
...
bool withheld(std::uint32_t key) noexcept {
    ensure_loaded();
    AcquireSRWLockShared(&g_lock);
    const bool result = should_withhold(g_published, key);
    ReleaseSRWLockShared(&g_lock);
    return result;
}
...
bool save(const KeySet& pending) noexcept {
    ...
    // Mapped keys go out in map order. Whatever else the set holds follows, so a hand-added key
    // survives a visit to the configuration.
    bool complete = true;
...
bool set_music(Music music) noexcept {
    ...
    const int used = std::snprintf(
        document.data(),
        document.size(),
        "# Tower music configuration. Tokens: auto festival dawning crimson solstice "
        "guardian_games.\n"
