...
template<class Writer> bool directive_record(Writer& w,bool active,std::uint32_t event) noexcept {
    ...
    for(unsigned i=0;i<4;++i) {
        if(!absent(w) || !absent(w)) { return false; }
        for(unsigned j=0;j<4;++j) { if(!w.write(0,32)) { return false; } }
