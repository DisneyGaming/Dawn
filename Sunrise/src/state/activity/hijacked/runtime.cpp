...
bool load() noexcept {
    ...
    std::call_once(once,[] {
        ...
        try {
            ...
            else { document=coo::script::MissionDocument::read(std::filesystem::path(path.data()).parent_path()/L"Sunrise"/L"scripts"/L"hijacked.lua",kProfile,error); }
        ...
        if(document) { std::snprintf(line.data(),line.size(),"ev=coo_script mission=hijacked result=loaded format=lua fnv1a64=%016llX path=Sunrise/scripts/hijacked.lua reload=next_process",static_cast<unsigned long long>(document->fingerprint())); }
