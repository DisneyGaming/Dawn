...
inline std::size_t body_bits(const Frame& f,std::uint32_t key,std::uint8_t type,std::uint16_t slot) noexcept {
    ...
    if(a->asset==kDialogueAsset) {return coo::native_presentation::kDialogueBits+(f.activeRow==coo::kNoDialogue?0U:64U);}
    ...
    if(type==4 && a->authority==0x8080992FU) {return 252;}
...
template<class Writer> bool write_body(Writer& w,const Frame& f,std::uint32_t key,std::uint8_t type,std::uint16_t slot) noexcept {
    ...
    if(type==65) {
        for(std::size_t i=0;i<std::size(kScans);++i) {if(a==kScans[i].link) {
    ...
    if(type==1) {const auto& p=kSpawns[spawn_index(a)];return coo::native_combatant::write_source(w,{key,s.generation,p.rule,
        static_cast<std::uint8_t>(s.active && s.prepared?(s.survivingRequested==UINT8_MAX?p.requested[0]:s.survivingRequested):0),(a==kBoss?kBossTactics[f.bossStage]:p.tactical),static_cast<std::uint8_t>(s.active && s.prepared?p.requested[1]:0),p.categories==2,true,false,s.retired});}
    if(type==4) {return coo::native_device::object(w,s.generation,s.active);}
