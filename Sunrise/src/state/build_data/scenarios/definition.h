...
/** Roster groups one destination publishes through bubble-local sub-blocks. */
inline constexpr std::size_t kDestinationBubbleGroupCapacity = 4;
/**
 * Content-derived authored groups cached for one selected slice, including its scenario root.
 * Towerfall's widest selected slice is one directive root plus four explicit-slice locals.
 */
...
inline constexpr std::size_t kDestinationWireGroupCapacity = 256;
...
/**
 * One roster group object and the descriptor-backed slots activity message 5 seeds for it.
 * Host-only slots have no client descriptor and are omitted, so the array ordinal is not always
 * the real slot index.
 */
struct RosterGroup {
    ...
    /** Package tag whose decoded blob physically carries each descriptor. */
    std::array<std::uint32_t, kRosterSlotCapacity> descriptorTags{};
    /** Byte offset of each descriptor inside its descriptorTags entry. */
    std::array<std::uint32_t, kRosterSlotCapacity> descriptorOffsets{};
    /** Native component class carried by each retained package descriptor. */
    std::array<std::uint32_t, kRosterSlotCapacity> componentClasses{};
    /** Sense schema carried by each descriptor, or the package absent-schema sentinel. */
    std::array<std::uint32_t, kRosterSlotCapacity> senseSchemas{};
    /** Authority schema carried by each descriptor, or the package absent-schema sentinel. */
    std::array<std::uint32_t, kRosterSlotCapacity> authSchemas{};
};
...
struct Definition {
    ...
    /** Used entries in the bubble-local group arrays. */
    ...
    /** Groups registered only while one of their bubble-mask bits is current. */
    std::array<std::uint16_t, kDestinationBubbleGroupCapacity> bubbleGroups{};
    /** Bubbles each bubble-local group belongs to, one bit per client bubble index. */
    std::array<std::uint64_t, kDestinationBubbleGroupCapacity> bubbleGroupMasks{};
    /**
     * Per-slice authored overlays. Element zero is the primary-registry scenario root; remaining
     * elements are same-package third-registry objects with an explicit ObjectBubble for that
     * slice. An empty row means extraction could not prove a complete, capacity-safe overlay.
     */
    std::array<std::uint8_t, kBubbleCapacity> authoredGroupCounts{};
    std::array<std::array<std::uint16_t, kDestinationAuthoredGroupCapacity>, kBubbleCapacity>
        authoredGroups{};
    /**
...
static_assert(kRosterGroupCapacity <= 0xFFFFU);
static_assert(kDestinationGroupCapacity + kDestinationBubbleGroupCapacity
