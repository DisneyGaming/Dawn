...
/** Copied State inputs consumed by roster construction without any State lookup. */
struct RosterSnapshotInputs final {
    state::activity::destination::DestinationSelection destination{};
    state::activity::defaults::ActivityDefaults defaults{};
    state::activity::membership::MembershipState sourceMembership{};
 ...
 */
[[nodiscard]] bool seed_identity(state::activity::ActivityInstanceKey activity,
 ...
 */
[[nodiscard]] bool seed_transition_token(state::activity::ActivityInstanceKey activity) noexcept;
 ...
 * byte destructs and rebuilds every object the roster entries own. A burst send moves it only
 * for a real mission-director activation edge.
...
/** Builds from one immutable copied region plan and mutates only the caller's Session candidate. */
