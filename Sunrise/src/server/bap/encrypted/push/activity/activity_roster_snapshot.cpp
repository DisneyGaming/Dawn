...
RosterOutcome build_roster_snapshot(Session& session,
    ...
    if(nativePublisher==native_publisher::Role::creator
        ...
        const auto frame=server::runtime::activity::native_activity::update(
            ...
            session.activity.rosterSends>=kWarmupSends,populationPrefetchBubble);
...
[[nodiscard]] bool make_membership_wire(
    state::activity::ActivityInstanceKey activity,bool validatedOmega,bool validatedBeyond,bool validatedGarden,bool validatedLaunchpad,bool validatedApproach,
