...
namespace sunrise::server::bap::encrypted::push::activity::hijacked_roster {
namespace native=state::activity::hijacked;
...
// Include native point-only groups as well: directive markers refer to their
// scoped type-31 sources. All14 recovered groups fit the existing wire capacity.
...
inline constexpr std::uint64_t kRootBubbles=0x1FFFFFFFFFFFULL;
...
[[nodiscard]] bool prepare_layout(layouts::Definition& layout,FindIndex findIndex,FindGroup find) noexcept {
    if(layout.tag!=native::kScenario || layout.nameLength!=15
        || std::string_view(layout.name.data(),layout.nameLength)!="adventure_rumba"
        || layout.bubbleCount!=45) { return false; }
    const native::Group* root{};
    for(const auto& group:native::kGroups) { if(group.key==native::kRoot) { if(root) { return false; }root=&group; } }
    if(!root) { return false; }
    // Extraction order changes as other activities add groups. Resolve the key
    // without copying every large descriptor record on the frame thread.
    layouts::RosterGroup verified{};std::uint16_t index{};
    if(!findIndex(root->key,index) || index>=layouts::kRosterGroupCapacity
...
[[nodiscard]] bool admit(const layouts::Definition& layout,Storage& storage,wire::Roster& roster,FindGroupByKey findByKey,std::uint32_t* failedKey=nullptr) noexcept {
    ...
    for(const auto& expected:native::kGroups) {
        ...
        // Copy only the matching row; stable-key lookup still requires every
        // descriptor identity check before any authority body is admitted.
        if(!findByKey(expected.key,group) || !matches(group,expected)) { return false; }
