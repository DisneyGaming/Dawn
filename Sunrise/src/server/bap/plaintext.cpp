...
void arm_encryption(Session& session, const state::BapState& bap) noexcept {
    ...
    session.authenticated = lifecycle::authentication_key_is_current(session);
}
...
bool consume(Session& session,
    ...
    // A repeated hello retires every nonce, binding and deferred publication owned by the old
    // authentication. The connection key and authentication clock survive that retirement.
    ...
    if (!lifecycle::allocate_authentication_key(session, encoded, authenticationKey)) {
        (void)state::matchmaking::release_context(matchmakingContext);
        if (written != 0) {
            SecureZeroMemory(response.data(), written);
            written = 0;
        }
        report_refusal(session,
                       frame.messageId,
                       encoded ? "authentication_generation" : "encode");
        return false;
    }
    // Publish the exact auth owner, context and counters only after the whole svc-26 response and
    // its non-wrapping generation exist. A context that was not free remains an invalid handle.
    session.authenticationKey = authenticationKey;
