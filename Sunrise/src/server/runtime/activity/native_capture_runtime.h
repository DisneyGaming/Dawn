...
public:
    ...
    template<class Definition,class Document> [[nodiscard]] bool begin(const Definition& d,const Document& doc) noexcept {
        if(!valid(d,doc))return false;
    ...
    [[nodiscard]] bool start(std::size_t index,coo::Token token) noexcept {
        if(index>=bindings_.size() || !clock_ || states_[index].requested || token.run!=clock_.domain.owner.incarnation.value)return false;
        ...
        state.ticket={clock_.domain,token,{p.registry->key,descriptor->descriptorTag,4,p.slot},p.generation,
            ...
            clock_.configuration,{true,{true,0,durations_[index],0,durations_[index],clock_.elapsedTicks,1.0F},false},0};
        if(!capture_bridge::bind(state.ticket))return false;
        state.requested=true;return true;
    }
    ...
    struct State {capture_feedback::Ticket ticket{};capture_feedback::Observation last{};bool requested{},ready{},completed{};};
