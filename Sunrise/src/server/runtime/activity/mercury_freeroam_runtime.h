...
inline constexpr std::array<Patrol,16> kPatrols{{
    {1,Faction::cabal,"pf_lighthouse_ca_pond_a",true},
    {7,Faction::vex,"pf_lighthouse_vx_center_left_b",false},
    {8,Faction::cabal,"pf_lighthouse_ca_blocks_right_a",false},
    {9,Faction::cabal,"pf_lighthouse_ca_cannon_forest_a",false},
    {10,Faction::cabal,"pf_lighthouse_ca_cannon_tower_a",false},
    {11,Faction::cabal,"pf_lighthouse_ca_crater_right_a",false},
    {12,Faction::vex,"pf_lighthouse_vx_center_right_a",false},
    {13,Faction::vex,"pf_lighthouse_vx_crater_cannon_a",false},
    {14,Faction::vex,"pf_lighthouse_vx_steps_a",false},
    {15,Faction::cabal,"pf_lighthouse_ca_cannon_mid_a_hotspot",true},
    {16,Faction::vex,"pf_lighthouse_vx_cannon_mid_a_hotspot",true},
    {17,Faction::cabal,"pf_lighthouse_ca_crater_cannon_a_hotspot",true},
    {18,Faction::vex,"pf_lighthouse_vx_crater_left_a_hotspot",true},
    {19,Faction::cabal,"pf_lighthouse_ca_forest_a_hotspot",true},
    {20,Faction::vex,"pf_lighthouse_vx_forest_a_hotspot",true},
    {21,Faction::vex,"pf_lighthouse_vx_pond_a_hotspot",true},
}};
struct Wave final {
...
[[nodiscard]] constexpr bool valid(const Configuration& value) noexcept {
    ...
    // Installed source rows expose at most three combatant choices per request.
    // Treating every choice as an admission is conservative: eight ordinary
    // patrols start at three and eight warfront/large-area patrols at four; the
    // largest war target is nine, for at most27 receipts in one generation.
    if(!value.normalPatrolRequests || !value.largePatrolRequests
        || static_cast<std::size_t>(value.normalPatrolRequests)*kMaximumAuthoredActorsPerRequest>kReceiptLedgerCapacity
        || static_cast<std::size_t>(value.largePatrolRequests+largestWave)*kMaximumAuthoredActorsPerRequest
            >kReceiptLedgerCapacity)return false;
    std::size_t ordinary{},large{};
    for(const auto& patrol:kPatrols)(patrol.largeArea?large:ordinary)++;
    const auto patrolRequests=value.populations
        ? ordinary*value.normalPatrolRequests+large*value.largePatrolRequests:0U;
...
private:
    struct PatrolState final {std::uint64_t clearAt{};bool started{},cooling{};};
    [[nodiscard]] std::uint8_t patrol_target(const Patrol& patrol) const noexcept {
        return patrol.largeArea?configuration_.largePatrolRequests:configuration_.normalPatrolRequests;
    }
    ...
    [[nodiscard]] bool start_patrols(population::Service& service,std::uint32_t bubble) noexcept {
        ...
        for(std::size_t i=0;i<kPatrols.size();++i)if(!states[i].started) {
            ...
            else if(!request(staged,capability,patrol_target(patrol),bubble))return false;
    ...
    [[nodiscard]] bool update_patrols(std::uint64_t now,population::Service& service,
        ...
        for(std::size_t i=0;i<kPatrols.size();++i) {
            ...
            if(!clear){state.cooling=false;continue;}
            ...
            if(!request(staged,capability,patrol_target(kPatrols[i]),bubble,true))return false;
            state.cooling=false;
        }
        service=std::move(staged);patrol_=states;return true;
    }
    ...
    [[nodiscard]] int wave_cleared(const population::Service& service,
        ...
        for(std::size_t side=0;side<2;++side) {
            ...
            const auto state=settled(service,ledgers[capability],capability,
                nativePending[capability]!=0,waveBaseline_[side]+1);
    ...
    [[nodiscard]] bool publish_wave(population::Service& service,std::span<const Ledger> ledgers,
        ...
        for(std::size_t side=0;side<2;++side) {
            ...
            waveBaseline_[side]=counts.admitted;
            const auto target=static_cast<unsigned>(staged.target(capability))+configuration_.waveRequests[wave];
            if(target>63 || !request(staged,capability,static_cast<std::uint8_t>(target),bubble))return false;
        }
        service=std::move(staged);wave_=wave;phase_=WarPhase::active;
    ...
    std::array<std::size_t,2> waveBaseline_{};
