...
using sunrise::server::bap::Session;
using sunrise::server::bap::ActivityBindingState;
using sunrise::server::bap::ActivityRecordCreator;
using sunrise::server::bap::ActivityRecordLease;
using sunrise::server::bap::RosterPublication;
using sunrise::server::bap::RegionLineage;
using sunrise::server::bap::RegionLineageKind;
using sunrise::server::bap::RegionPublicationDebt;
...
void connection_clock_outlives_reusable_session_storage() {
    ...
    CHECK(sunrise::server::bap::lifecycle::allocate_connection_key(
...
void failed_service26_encoding_does_not_advance_authentication() {
    ...
    CHECK(!sunrise::server::bap::lifecycle::allocate_authentication_key(
...
void successful_service26_allocates_an_exact_owner_chain() {
    ...
    CHECK(static_cast<bool>(session.authenticationKey));
    CHECK(sunrise::server::bap::lifecycle::authentication_key_is_current(session));
...
void reauthentication_retains_only_connection_owned_state() {
    ...
    const auto connection =
        sunrise::server::bap::lifecycle::connection_owned_state(session);
    sunrise::server::bap::lifecycle::restore_connection_owned_state(session, connection);
...
void allocation_and_join_have_distinct_lease_rules() {
    namespace lifecycle = sunrise::server::bap::lifecycle;
...
void fresh_postcommit_guard_rolls_back_only_an_untransferred_fresh_allocation() {
    namespace transactions = sunrise::server::bap::encrypted::transactions;
...
void binding_candidate_is_owned_by_authentication_and_publishes_nothing() {
    ...
    CHECK(sunrise::server::bap::lifecycle::stage_activity_binding_key(session, candidate));
    ...
    CHECK(sunrise::server::bap::lifecycle::is_staged_activity_binding_key(session, candidate));
...
void binding_candidates_advance_under_one_authentication() {
    ...
    CHECK(sunrise::server::bap::lifecycle::stage_activity_binding_key(session, first));
    session.activityBindingClock = first.generation;
    CHECK(sunrise::server::bap::lifecycle::stage_activity_binding_key(session, second));
    CHECK(first.generation.value == kFirstGeneration);
...
void every_successful_same_activity_join_advances_binding_and_preserves_only_role() {
    ...
    firstJoin.joinedForeignSession =
        sunrise::server::bap::lifecycle::joined_foreign_session_after_replacement(
...
void foreign_role_is_set_on_different_join_preserved_on_same_join_and_cleared_on_allocate() {
    ...
    const ActivityInstanceKey first{70, ActivityIncarnation{1}};
    CHECK(sunrise::server::bap::lifecycle::joined_foreign_session_after_replacement(
        unbound, first, true));
...
void roster_publication_candidates_do_not_advance_until_published() {
    ...
    CHECK(sunrise::server::bap::lifecycle::stage_roster_publication_generation(
        binding, candidate));
    CHECK(candidate.value == kFirstGeneration);
    CHECK(binding.rosterPublicationClock.value == kAbsentGeneration);
    CHECK(!sunrise::server::bap::lifecycle::staged_roster_is_current(binding));
...
void stale_roster_publication_cannot_match_or_rollback_a_successor() {
    ...
    CHECK(sunrise::server::bap::lifecycle::staged_roster_is_current(predecessor));
    ...
    CHECK(!sunrise::server::bap::lifecycle::staged_roster_is_current(successor));
    if (!sunrise::server::bap::lifecycle::staged_roster_is_current(successor)) {
