...
void mars_survivors() {
    ...
    a::open_world::Director director;CHECK(director.begin(owner,99,*definition.openWorld,definition.populations));
    ...
    const auto& cap=definition.populations[index];
...
void all_destination_sources() {
    for(const auto* definition:a::open_world::profiles::kActivities) {
        ...
        CHECK(director.begin(owner,99,*definition->openWorld,definition->populations));
        std::array<c::NativePopulationLedger<64>,a::population::kSourceCapacity> ledgers{};
        std::array<std::uint8_t,a::population::kSourceCapacity> pending{};
        ...
        for(std::size_t index=0;index<definition->populations.size();++index) {
        ...
        for(std::size_t i=0;i<definition->populations.size();++i)CHECK(!service.renewal(i).pending);
        CHECK(tick(31000,63));
        for(std::size_t i=0;i<definition->populations.size();++i) {
...
int main(){deadlines();categories();mars_survivors();all_destination_sources();std::printf("PASS %u patrol replenishment checks\n",checks);}
