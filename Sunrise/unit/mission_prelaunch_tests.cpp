...
namespace f = sunrise::state::activity::forced;
namespace d = sunrise::state::activity::destination;
...
int main() {
    ...
    check(f::publish(f::profiles::kHauntedForestOpening), "Haunted Forest preset rejected");
    check(!f::override_active(), "Haunted Forest activated before native selection");
    check(p::find("infinite_abyss") == &p::kHauntedForest, "Haunted Forest profile not registered");
    check(p::kHauntedForest.activity == 78
              && p::kHauntedForest.investmentHash == 0x56B7B6A5U
              && p::kHauntedForest.packageHash == 0x6DA20650U
              && p::kHauntedForest.activityTag == 0x81550000U
              && p::kHauntedForest.launchTag == 0x80F9FDD2U
              && std::string_view(p::kHauntedForest.event) == "haunted_forest_direct",
          "Haunted Forest profile constants drifted");
    auto hauntedChosen = descriptor(282,"mission_red_legion");
    check(!f::apply(hauntedChosen) && hauntedChosen.activityIndex == 282,
        "Haunted Forest used late Chosen rewrite");
    check(!f::commit_prelaunch_authored_selection(20,282,selected),
        "Haunted Forest accepted foreign donor");
    check(!f::commit_prelaunch_authored_selection(282,78,selected),
        "Haunted Forest accepted mixed donor");
    check(f::commit_prelaunch_authored_selection(282,282,selected),
        "Haunted Forest exact Chosen donor rejected");
    check(p::configured(selected) == &p::kHauntedForest && f::override_active(),
        "wrong Haunted Forest committed identity");
    auto hauntedRoute = descriptor(78,"infinite_abyss");
    auto firewalledRoute = descriptor(79,"infinite_abyss");
    check(p::matches(p::kHauntedForest,hauntedRoute.previousActivityIndex,
                    hauntedRoute.activityIndex,"infinite_abyss"),
        "Haunted Forest native route not required");
    check(!p::matches(p::kHauntedForest,firewalledRoute.previousActivityIndex,
                     firewalledRoute.activityIndex,"infinite_abyss"),
        "Firewalled activity accepted for Haunted Forest");
    check(!p::matches(p::kHauntedForest,78,79,"infinite_abyss"),
        "mixed Haunted Forest native route accepted");
    check(f::apply(hauntedRoute), "native Haunted Forest route rejected");
    check(hauntedRoute.activityIndex == 78 && hauntedRoute.previousActivityIndex == 78,
        "Haunted Forest indices drifted");
    check(hauntedRoute.arrivalBubbleOverride == 13 && hauntedRoute.sliceSetOverride == 104,
        "Haunted Forest arrival region wrong");
    check(hauntedRoute.spawnSetOverride == 0x79E3AB1FU && hauntedRoute.hasSpawnSetOverride,
        "Haunted Forest opening spawn lost");
    check(hauntedRoute.descriptorBitLength == 620 && hauntedRoute.descriptorBits[60] == std::byte{0xA5},
        "Haunted Forest authored descriptor tail changed");
    ...
    std::puts("Mission prelaunch: Gateway/Haunted staging and wire contracts, Towerfall compatibility, Omega isolation passed.");
}
