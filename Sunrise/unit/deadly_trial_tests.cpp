...
void roster() {
    ...
    records::ScenarioRecord captured{};std::ifstream input("Sunrise/unit/fixtures/deadly_trial_launch_layout.bin",std::ios::binary);
...
void presentation_and_overpass(const c::script::Views& views) {
    namespace native=sunrise::client::hooks::bootflow::deadly_trial_presentation;
    ...
    header(true);f.generations[1]=7;f.activeRow=1;
    native::put<std::uint32_t>(bytes,0x118C,6);CHECK(native::dialogue_records(bytes,f));
    CHECK(native::read<std::uint32_t>(bytes,0x1C0)==7);CHECK(native::read<std::uint8_t>(bytes,0x1C4)==2);
    CHECK(native::read<std::uint32_t>(bytes,0x118C)==6);CHECK(!native::dialogue_records(bytes,f));
    f.activeRow=c::kNoDialogue;CHECK(native::dialogue_records(bytes,f));CHECK(native::read<std::uint64_t>(bytes,0x1B0)==0);
    CHECK(native::read<std::uint32_t>(bytes,0x118C)==6);
    ...
    for(const auto type:{1U,2U}) {
        ...
        std::ifstream fixture(type==1?"Sunrise/unit/fixtures/deadly_trial_skiff_source.bin":"Sunrise/unit/fixtures/deadly_trial_skiff_flight.bin",std::ios::binary);
...
void pike_mount(const c::script::Views& views) {
    namespace native=sunrise::client::hooks::bootflow::coo_native;
...
int main(int argc,char** argv) {
    ...
    namespace revival=sunrise::client::hooks::bootflow::deadly_trial_revival;
