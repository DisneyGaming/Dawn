...
namespace wire=sunrise::middleware::bap::activity_message::native::activity_clock;
...
void publication_tests() {
    ...
    for(unsigned mode=0;mode<24;++mode) {
        ...
        std::array<std::byte,sunrise::state::kBapNonceSize> nonce{};nonce[0]=std::byte{7};const auto initialNonce=nonce;
    ...
    for(std::size_t cap=0;cap<35;++cap) {
        ...
        std::array<std::byte,sunrise::state::kBapNonceSize> nonce{};push::BorrowedClockPublication publication{};
