...
namespace hf=sunrise::server::runtime::activity::haunted_forest;
namespace dest=sunrise::state::activity::destination;
namespace scenario=sunrise::state::build_data::scenarios;
...
int main(int argc,char**argv) {
 ...
 static std::array<sunrise::state::build_data::activities::Definition,4095> catalog{};std::size_t count{};
 ...
 snapshot.omegaSceneAuthority=true;snapshot.omegaCrownRestriction=sunrise::state::activity::omega_crown_respawn::Restriction::enable;CHECK(ordinal(body(snapshot))==14);
 snapshot.archiveOmega=false;snapshot.omegaCrownRestriction=sunrise::state::activity::omega_crown_respawn::Restriction::keep;snapshot.omegaMission.generation=1;snapshot.omegaMission.restriction=true;CHECK(ordinal(body(snapshot))==14);
