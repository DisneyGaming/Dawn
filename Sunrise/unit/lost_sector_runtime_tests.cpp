...
int main() {
    ...
    for(std::uint32_t generation=1;generation<=40;++generation) {
        ...
        const auto slot=sunrise::server::bap::encrypted::lost_sector_rewards::detail::select_claim_slot(
    ...
    CHECK(sunrise::server::bap::encrypted::lost_sector_rewards::detail::select_claim_slot(
        interleaved,[](const ClaimSlot& claim) {return claim.occupied;},
        [](const ClaimSlot&) {return false;},[](const ClaimSlot&) {return false;})
        ==sunrise::server::bap::encrypted::lost_sector_rewards::detail::kNoClaimSlot);
    CHECK(sunrise::server::bap::encrypted::lost_sector_rewards::detail::select_claim_slot(
        interleaved,[](const ClaimSlot& claim) {return claim.occupied;},
        [](const ClaimSlot&) {return false;},
        [](const ClaimSlot& claim) {return claim.generation==7;})
        ==sunrise::server::bap::encrypted::lost_sector_rewards::detail::kDuplicateClaimSlot);
    ...
    CHECK(!ls::source_defeated(false,true,true,false,2,2,1));
    {
        ...
        sunrise::middleware::bap::activity_message::object_sense::Output use{};
    ...
    const auto membersCover=[](const auto& capabilities) {
        for(const auto& capability:capabilities) {
            sunrise::state::activity::coo::Asset asset{capability.registry->key,0,1,capability.slot};
            ...
            if(!sunrise::state::activity::open_world_members::registered(
    ...
    // additionally requires four live health-qualified crystal destructions.
    {
        ...
        sunrise::middleware::bap::activity_message::native::world_device::Batch devices{};
