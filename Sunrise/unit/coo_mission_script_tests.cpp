...
int main() {
    std::ifstream input("Sunrise/unit/fixtures/mission_script_alternate.json",std::ios::binary);CHECK(input.good());
    ...
    auto duplicateCaps=std::vector<s::Capability>(std::begin(capabilities),std::end(capabilities));duplicateCaps.push_back(duplicateCaps[0]);invalidProfile=profile;invalidProfile.capabilities=duplicateCaps;CHECK(!s::MissionDocument::parse(text,invalidProfile,error));
    ...
    std::printf("PASS: %u checks; independent profile, forward references, parallel native receipts, added JSON steps, stale tokens, rejection and document isolation\n",checks);
}
