...
using sunrise::client::hooking::detour::UninstallResult;
namespace support = sunrise::client::hooks::bootflow::prologue_filler_ready_test_support;
...
// unchanged.
#define SUNRISE_PROLOGUE_FILLER_READY_TESTING 1
...
namespace bootflow = sunrise::client::hooks::bootflow;
...
[[nodiscard]] std::filesystem::path find_repository_root() {
    ...
    for (std::uint32_t depth = 0U; depth < 8U; ++depth) {
        if (std::filesystem::exists(cursor
                                    / "Sunrise/src/client/hooks/bootflow"
...
void source_call_graph_keeps_retail_install_and_checked_bootflow_propagation() {
    ...
    const std::string retail =
        read_source(root / "Sunrise/src/client/hooks/retail_log/retail_log_enqueue_observer.cpp");
    const std::string lifecycle =
        read_source(root / "Sunrise/src/client/hooks/bootflow/bootflow_hook_lifecycle.cpp");
