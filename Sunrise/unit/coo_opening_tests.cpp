...
namespace intake = sunrise::server::bap::encrypted::activity_message::omega_opening_intake;
...
static sunrise::server::bap::Session resetSession;
__declspec(noinline) void execute_after_session_reset() {
    ...
    for (unsigned mode = 0; mode < 3; ++mode) {
        ...
        if (mode == 0) { resetSession = sunrise::server::bap::Session{}; }
