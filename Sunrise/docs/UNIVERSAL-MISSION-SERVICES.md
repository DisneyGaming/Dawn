...
The Vance cast is requested on arrival. The invitation is submitted at Lighthouse entry; the native turn event follows that submission. Approach remains latched while the turn and greeting finish. The actual conversation-start receipt starts the JSON clocks: 22,640 ms to raise the Lighthouse, 31,000 ms to complete the conversation and then the mission. The NPC's longer idle scene does not block completion.

`gateway.json` now selects Forest gate, Lighthouse portal, cube and Vance markers through `presentation.markers`. Its enemy populations, opening graph, ending dependencies and dialogue rows remain unchanged by this migration. Omega retains its accepted bindings and JSON; adding shared capabilities does not opt it into new readiness policies.
