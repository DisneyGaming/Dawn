...
Sunrise keeps durable player state in `Sunrise/player-state.db`, relative to the loaded
`steam_api64.dll`. SQLite is compiled into the module; players do not install a database server or
an SQLite program.

On the first boot without a database, Sunrise imports the account, inventory, equipment, sockets,
...
newer-schema database stops startup with an error. Sunrise does not silently reset it or fall back
...
Sunrise records completion and the checkpoint identities exposed by reconstructed controllers.
...
and world state as one coherent operation. Sunrise therefore does not fake resume by changing a
...
configuration. Repository defaults live in `Sunrise/resources/default_*.json`. The repository
