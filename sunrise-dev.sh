#!/usr/bin/env bash
# Linux counterpart of sunrise-dev.ps1. Builds a Windows x64 DLL for Wine/Proton.
...
#
#   ./sunrise-dev.sh --game-root /path/to/your/Sunrise
#   ./sunrise-dev.sh --setup-only
#   ./sunrise-dev.sh --build-only --config Debug
#   ./sunrise-dev.sh --game-root /path/to/Sunrise --restore
#   ./sunrise-dev.sh --game-root /path/to/Sunrise --skip-build --clear-cache
#
...
# Close Sunrise before installing/restoring. A best-effort process-name check
...
def locked(directory):
    ...
    with (directory / ".sunrise-dev.lock").open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError:
            fail(f"Another sunrise-dev operation is using {directory}")
        yield
...
def download_checked(url, destination, expected):
    ...
    for attempt in range(3):
        ...
        temporary = Path(name)
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "Dawn-sunrise-dev"})
...
def xwin_network():
    ...
    authorization = b"Basic " + base64.b64encode(f"sunrise:{token}".encode())
    ...
    with Server(("127.0.0.1", 0), Tunnel) as server:
        ...
        try:
            ...
            yield ["--https-proxy", f"http://sunrise:{token}@127.0.0.1:{server.server_address[1]}"]
        finally:
            server.shutdown()
            thread.join()
...
def build(repo, work, args):
    project = repo / "Sunrise/Sunrise.vcxproj"
    ...
    cm = ["cmake_minimum_required(VERSION 3.25)", "project(SunriseLinux C CXX RC)",
    ...
    dirs = [repo / "Sunrise" / p for p in ("src", "resources", "vendor/lua/src", "vendor/detours", "vendor/imgui", "vendor/imgui/backends")]
...
def replace_dll(source, destination):
    ...
    validate_dll(source)
    fd, name = tempfile.mkstemp(prefix=".sunrise-dev-", dir=destination.parent)
    temporary = Path(name)
    try:
...
def validate_game_root(game):
    if not (game / "destiny2.exe").is_file() or not (game / "bin/x64").is_dir():
        fail(f"Not a game install: {game}. Select the Sunrise folder containing destiny2.exe and bin/x64/.")
    if not ((game / "bin/x64/Sunrise").is_dir() or (game / ".sunrise/original/steam_api64.dll").is_file()):
        fail(f"No existing Sunrise installation found in {game}. --game-root must point to your Sunrise install.")


def deploy(game, dll, args):
    validate_game_root(game)
    live = game / "bin/x64/steam_api64.dll"
    runtime = live.parent / "Sunrise"
    backup = game / ".sunrise/backup"
    ...
    with locked(game / ".sunrise"):
        ...
        if (game / ".sunrise/install-state.json").exists():
            print("Note: rerunning the Sunrise installer may replace this local build.")
        step("Done. You can launch the game when ready.")


def main():
    parser = argparse.ArgumentParser(prog="sunrise-dev.sh", description="Build and install the Dawn Windows x64 DLL on Linux. Windows SDK setup is automatic.",
        epilog="Normal use: ./sunrise-dev.sh --game-root \"/path/to/your/Sunrise\"\n"
               "First-time SDK preparation only: ./sunrise-dev.sh --setup-only\n"
               "Rollback: ./sunrise-dev.sh --game-root \"/path/to/your/Sunrise\" --restore\n\n"
               ...
               "Default SDK cache: $XDG_CACHE_HOME/sunrise-dev (otherwise ~/.cache/sunrise-dev).\n"
               "Only the proxy DLL is deployed, matching sunrise-dev.ps1. Mission scripts are managed separately.\n"
               "Close Sunrise before installing/restoring. A best-effort process-name check is used when available;\n"
    ...
    usual.add_argument("--game-root", help="REQUIRED for install/restore: your Sunrise folder containing destiny2.exe")
    ...
    advanced.add_argument("--clear-cache", "-ClearCache", action="store_true", help="clear generated Sunrise/cache/*.bin; next game startup rebuilds them")
    advanced.add_argument("--repo", default=os.environ.get("SUNRISE_REPO"), help="use a different Dawn checkout (or SUNRISE_REPO); normally inferred from script location")
    advanced.add_argument("--xwin", default=os.environ.get("SUNRISE_XWIN"), help="use an already prepared Windows SDK instead of automatic setup (or SUNRISE_XWIN)")
    cache_home = Path(os.environ.get("XDG_CACHE_HOME") or Path.home() / ".cache")
    advanced.add_argument("--cache-dir", default=str(cache_home / "sunrise-dev"), help="store automatic SDK/tools/downloads somewhere else; useful if home space is limited")
    ...
    if not (args.build_only or args.setup_only) and not args.game_root:
        parser.error('an explicit --game-root "/path/to/your/Sunrise" is required for every install or restore')
    ...
    if args.repo:
        ...
        repo = here if (here / "Sunrise/Sunrise.vcxproj").is_file() else here / "Sunrise-src"
    game = Path(args.game_root).expanduser().resolve() if args.game_root else None
    if args.setup_only:
        build_tools()
        prepare_sdk(args)
        step('Setup complete. Install with: ./sunrise-dev.sh --game-root "/path/to/your/Sunrise"')
        return
    if game is not None and not args.build_only:
        validate_game_root(game)
        step(f"Selected Sunrise install: {game}")
    if args.restore:
