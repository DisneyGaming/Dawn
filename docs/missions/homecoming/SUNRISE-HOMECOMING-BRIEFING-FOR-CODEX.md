# Sunrise / Destiny 2 "Homecoming" — full technical briefing
...
**Sunrise is an offline / archival mod for Destiny 2.** Destiny 2 is an always-online game whose
servers for this content are gone. Sunrise makes a specific archived build run **fully offline** by:
...
- **Emulating the online backend locally** — Bungie's "BAP" services, Demonware, the activity/session
  ...
  talking to Sunrise inside its own process / loopback.
...
**unpacked memory dump** (see §9), and all runtime instrumentation is done from **inside Sunrise's own
...
Sunrise's source lives under `Sunrise-src/Sunrise/src/`. Two halves matter:
...
The repo (`Sunrise-src`, a git repo) has diverged lines of work:
...
- **`spawner`** — an older, **divergent** line (cannot be git-merged; different base commit). It holds a
  **working Entity Spawner** (spawn arbitrary entities into the world via the overlay), plus older
  ...
  (`_Sunrise-src_spawner_backup_20260818`). Important because the **Entity Spawner is very likely "the
...
mission inert
 └ authored manager never ACTIVATES  (manager+0x1AF00 == 0 for every manager)
    └ the client only activates from a session-stream message of KIND 22 ("host-reestablish")
       └ Sunrise never sent kind 22  →  and when triggered, it is host-migration and is REFUSED
```
...
When it's triggered it is **refused by the current host** (observed live). Since Sunrise is *both* the
...
Sunrise keeper called **`region_force_public`**, not to the authored-launch machinery.
...
1. **Sunrise-as-activity-host simulates the authored content** — i.e. Sunrise's server side sends the
   encounter/spawn/script/roster data a real Bungie activity host would, and the host-migration (kind
   22) handshake is made to **succeed** (Sunrise, as the host, must accept it — currently it refuses).
...
- **Unpacked dump:** `C:\Destiny 2 Development\destiny2_unpacked.bin` (~145 MB full memory image; every
  ...
  channel is Sunrise's own in-process hooks.
...
```
- Builds `Sunrise-src\Sunrise.sln`, deploys to `bin\x64\steam_api64.dll` (backs up the old one).
...
- **Log:** `bin\x64\Sunrise\logs\sunrise.log` (client/core at `debug`, `file_sink` on). The game's own
...
field`. All fields are data Sunrise already encodes in `publish_snapshot()` in
...
currently *refused*. So the fuller task is: (i) send a well-formed kind-22, **and (ii) make Sunrise (the
...
- `sunrise.log` — the live diagnostic.

---

### One-paragraph orientation

Sunrise is an offline Destiny 2 mod (a `steam_api64.dll` proxy that hooks the client and fakes the
...
(session-stream **kind 22**, "host-reestablish") that Sunrise never sent and that is **refused** when
...
Sunrise **simulate the activity host** (and accept the kind-22 migration), or use the **client-side
