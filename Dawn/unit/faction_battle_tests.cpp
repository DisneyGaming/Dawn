...
namespace f=dawn::server::runtime::activity::faction_battle;
namespace p=dawn::server::runtime::activity::population;
namespace r=dawn::server::runtime::activity::registry;
namespace e=dawn::state::activity::native_population;
...
static void validation() {
    ...
    f::Service service;CHECK(!service.begin(token,invalid));CHECK(service.project(3).count==0);
    CHECK(!dawn::server::runtime::activity::mercury::faction_battle_evidence::kActivationSupported);
    CHECK(dawn::server::runtime::activity::mercury::faction_battle_evidence::kUnresolved.size()==8);
}
...
static void consumed(p::Service& service,std::size_t capability,std::uint32_t revision) {
    const auto projected=service.project(15);const auto& cap=
        dawn::server::runtime::activity::mercury::kPopulations[capability];
    for(std::size_t i=0;i<projected.count;++i)
...
static void mercury_schedule_and_patrols() {
    ...
    for(const auto& patrol:fr::kPatrols) {
        ...
        const auto* group=dawn::state::activity::coo::mercury::ambient::find(capability.registry->key);
    ...
    for(const auto& pin:defenderPins) {
        ...
        const dawn::state::activity::open_world_members::SourceRange* range=nullptr;
        for(const auto& candidate:dawn::state::activity::open_world_members::kSources)
        ...
        for(std::uint32_t i=0;i<range->count;++i)
            CHECK(dawn::state::activity::open_world_members::kChoices[range->first+i].entity
