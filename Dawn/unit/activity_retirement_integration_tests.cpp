...
namespace matchmaking = dawn::state::matchmaking;
...
void reset_state_storage() noexcept {
    AcquireSRWLockExclusive(&dawn::state::runtime::storage::g_stateLock);
    dawn::state::runtime::storage::g_state = dawn::state::State{};
    ReleaseSRWLockExclusive(&dawn::state::runtime::storage::g_stateLock);
}
...
std::size_t occupied_state_records() noexcept {
    std::size_t count = 0;
    AcquireSRWLockShared(&dawn::state::runtime::storage::g_stateLock);
    for (const activity::SessionRecord& record :
         dawn::state::runtime::storage::g_state.activity.sessions) {
        count += record.occupied ? 1U : 0U;
    }
    ReleaseSRWLockShared(&dawn::state::runtime::storage::g_stateLock);
    return count;
}
...
bool bap_event(dawn::client::network::BapEvent event, std::uint32_t id) noexcept {
    dawn::client::network::BapRequest request{};
    request.event = event;
    request.connectionId = id;
    dawn::client::network::BapResponse response{};
...
void open_owned(std::uint32_t id,
    ...
    CHECK(bap_event(dawn::client::network::BapEvent::open, id));
...
void fence_removal_stale_precheck_uses_real_claim() {
    ...
    CHECK(bap_event(dawn::client::network::BapEvent::close, 1));
...
void claim_pin_eviction_allocation_reset_and_pending_drain_honor_real_fence() {
    ...
    std::thread retirement([&]() noexcept {
        closed = bap_event(dawn::client::network::BapEvent::close, 1);
        done.signal();
    });
    barrier.wait_until_reached();
...
void pinned_partial_cascade_drains_at_real_event_tail() {
    reset_fixture();
    PartialCascadeFixture fixture = make_partial_cascade_fixture();
    (void)bap_event(dawn::client::network::BapEvent::close, 1);
    check_partial_cascade_blocked(fixture);
    g_eventLease.store(&fixture.lease, std::memory_order_release);
    CHECK(bap_event(dawn::client::network::BapEvent::open, 4));
    CHECK(!fixture.lease.pinned);
    CHECK(occupied_state_records() == 0);
    CHECK(group::test_support::snapshot().fenceCount == 0);
    CHECK(bap::test_support::snapshot().pendingCount == 0);
    ...
    CHECK(retirement_count(fixture.second, activity::RetireResult::retired) == 1);
    CHECK(retirement_count(fixture.pinnedChild, activity::RetireResult::retired) == 1);
    CHECK(bap_event(dawn::client::network::BapEvent::close, 4));
    check_conservation();
}
...
void close_and_open_quarantine_only_real_failed_matchmaking_context() {
    ...
    CHECK(!bap_event(dawn::client::network::BapEvent::poll, 1));
    ...
    matchmaking::test_support::fail_next_releases(1);
    CHECK(!bap_event(dawn::client::network::BapEvent::open, 1));
...
void borrower_invalidation_and_lock_order_stress_reach_final_drain() {
    ...
    CHECK(bap_event(dawn::client::network::BapEvent::open, 2));
