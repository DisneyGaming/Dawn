...
namespace catalog=dawn::state::activity::coo::lost_sector;
...
int main() {
    ...
    for(const auto& group:dawn::state::activity::coo::edz_moon_lost_sector_groups::kRegistryGroups) {
        ++checks;
        if(!group.scenario || !group.object || !group.key || !group.sliceMask
            || (group.sliceMask&(group.sliceMask-1))
    ...
    if(dawn::state::activity::coo::edz_moon_lost_sector_groups::required(
