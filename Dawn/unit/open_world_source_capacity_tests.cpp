...
namespace pending=dawn::client::hooks::bootflow::native_population_pending;
...
struct Fixture final {
    ...
    std::array<std::array<dawn::state::activity::coo::registry::Slot,6>,groups> slots{};
    std::array<dawn::state::activity::coo::registry::Definition,groups> registries{};
...
events::Lease lease(std::uint16_t slot=0) {
    const dawn::state::activity::ActivityInstanceKey activityKey{0x1234U,{7}};
...
int main() {
    ...
    static_assert(dawn::client::hooks::bootflow::native_population_streaming::kSourceCapacity==512);
    static_assert(wire::kGroupCapacity==256);
    static_assert(dawn::state::build_data::scenarios::kDestinationWireGroupCapacity==256);
    ...
    std::printf("PASS %u open-world source-capacity checks wire=%zu service=%zu director=%zu snapshot=%zu roster_group=%zu\n",
        ...
        sizeof(wire::Snapshot),sizeof(dawn::state::build_data::scenarios::RosterGroup));
}
