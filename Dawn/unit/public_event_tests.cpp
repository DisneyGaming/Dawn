...
namespace mercury=dawn::server::runtime::activity::mercury::public_events;
namespace placement=dawn::server::runtime::activity::placement;
namespace coo=dawn::state::activity::coo;
namespace reg=dawn::state::activity::coo::registry;
namespace {
...
void raw_sense() {
    namespace sense=dawn::middleware::bap::activity_message::native::public_event_sense;
    struct Reader {
...
void packages(const char* directory) {
    ...
    for(const auto& group:mercury::kRegistries) {
        CHECK(dawn::server::runtime::activity::registry::valid(group));
