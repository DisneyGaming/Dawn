...
#include <fstream>
#include <algorithm>
using namespace dawn::state::activity::coo;
namespace sc=dawn::state::activity::coo::script;
unsigned checks{};
...
int main(int argc,char** argv) {
    ...
    if(argc==2 && std::string_view(argv[1])=="--invalid-admission") {
        ...
        dawn::state::activity::omega_presentation::Run presentation;presentation.initialize(1,true);
    ...
    std::ifstream file("Dawn/scripts/omega.lua",std::ios::binary);CHECK(file.good());
    ...
    namespace p=dawn::state::activity::omega_presentation;
    ...
    CHECK(!sc::Document::read("Dawn/scripts/does-not-exist.lua",error));
...
authored.presentation.binding_tables.forest.dialogue[1].delay_ms=2250
    ...
    namespace nativeEnding=dawn::state::activity::omega_ending;
