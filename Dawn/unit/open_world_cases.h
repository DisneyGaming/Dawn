...
inline void open_world_cases() {
    ...
    constexpr std::array<const char*,8> scripts{{
        "Dawn/scripts/eden_freeroam.json","Dawn/scripts/fleet_freeroam.json",
        "Dawn/scripts/polaris_freeroam.json","Dawn/scripts/planet_x_freeroam.json",
        "Dawn/scripts/tangled_shore_freeroam.json","Dawn/scripts/dreaming_city_freeroam.json",
        "Dawn/scripts/edz_freeroam.json","Dawn/scripts/luna_freeroam.json",
    }};
    ...
    for(std::size_t world=0;world<profiles::kActivities.size();++world) {
        ...
        CHECK(definition.registries.size()+definition.lostSectorRegistries.size()+5
            <=dawn::middleware::bap::activity_message::sensor_auth_update::kGroupCapacity);
        ...
        for(const auto& registry:definition.registries) {
            ...
            CHECK(authored::required(registry.scenario,registry.objectTag,registry.key,
                ...
                || dawn::state::activity::coo::edz_moon_lost_sector_groups::required(
        ...
        auto document=dawn::state::activity::coo::script::MissionDocument::parse(text,*definition.profile,error);
        ...
        std::array<dawn::state::activity::coo::NativePopulationLedger<64>,activity::population::kSourceCapacity> ledgers{};
        ...
        for(std::size_t bubble=0;bubble<bubbles.size();++bubble)if(bubbles[bubble]) {
            CHECK(director.update(now++,static_cast<std::uint32_t>(bubble),true,service,
                std::span<const dawn::state::activity::coo::NativePopulationLedger<64>>(ledgers),
        ...
        if(world==1) {
            ...
            struct Ledger final {
                dawn::state::activity::coo::PopulationCounts value{};
                [[nodiscard]] dawn::state::activity::coo::PopulationCounts counts() const noexcept {return value;}
            };
            ...
            for(std::size_t i=0;i<ordinary.size();++i) {
                ...
                dawn::middleware::bap::activity_message::sense_update::SenseObject consumed{};
        ...
        namespace native=dawn::state::activity::coo::native_combatant;
        std::array<std::byte,128> actual{},expected{};
        dawn::middleware::encoding::bits::Writer actualWriter(actual),expectedWriter(expected);
