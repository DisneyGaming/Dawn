...
int main(int argc, char** argv) {
    namespace hook = dawn::client::hooking;
    ...
    for (const auto& binding : dawn::client::hooks::bootflow::omega_portal_probe::kBindings) {
    ...
    for (const auto& binding : dawn::client::hooks::bootflow::omega_reveal_native::kBindings) {
    ...
    for (unsigned pass = 0; pass < 3; ++pass) {
        ...
        namespace reveal=dawn::client::hooks::bootflow::omega_reveal_native;
    ...
    std::printf("PASS: %zu pinned hooks in 3 cycles of 29+21+1 atomic transactions; twenty-one reveal rollback positions; %zu reveal and %zu portal signatures; publication wait/notify; no game code executed\n",
        targets.size(), dawn::client::hooks::bootflow::omega_reveal_native::kBindings.size(),
        dawn::client::hooks::bootflow::omega_portal_probe::kBindings.size());
}
