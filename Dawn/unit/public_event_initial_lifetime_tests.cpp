...
#include <iostream>
#include <cstdlib>
namespace rt=dawn::server::runtime::activity;namespace pe=rt::public_event;namespace mc=rt::mercury;
namespace coo=dawn::state::activity::coo;namespace msg=dawn::middleware::bap::activity_message;
...
std::vector<std::byte> bytes(const Frame& f){
 std::vector<std::byte> out(4096);dawn::middleware::encoding::bits::Writer w(out);
