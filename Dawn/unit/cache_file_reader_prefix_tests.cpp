...
using dawn::state::build_data::BuildIdentity;
using dawn::state::build_data::cache::LoadStatus;
namespace cache = dawn::state::build_data::cache;
...
struct ReaderFixture final {
    ...
    std::array<dawn::state::content::Definition, 1> named{};
    std::array<dawn::state::build_data::items::Definition, 1> items{};
    std::array<dawn::state::build_data::collectibles::Definition, 1> collectibles{};
    std::array<dawn::state::build_data::material_requirements::Definition, 1>
        materialRequirements{};
    std::array<dawn::state::build_data::items::socket_plugs::Rule, 1> socketPlugRules{};
    std::array<dawn::state::build_data::items::socket_plugs::Pool, 1> socketPlugPools{};
    std::array<dawn::state::build_data::inventory::buckets::Descriptor, 1> inventoryBuckets{};
    std::array<dawn::state::build_data::socket_entry_lists::Definition, 1> socketEntryLists{};
    std::array<dawn::state::build_data::progressions::Definition, 1> progressions{};
    std::array<dawn::state::build_data::scenarios::Definition, 1> scenarios{};
...
void cng_file_sha256_matches_known_vector(TemporaryCacheFile& file) {
    ...
    constexpr dawn::core::provenance::Sha256Digest expected{
    ...
    dawn::core::provenance::Sha256Digest actual{};
    CHECK(dawn::core::provenance::file_sha256(file.path(), actual));
    CHECK(actual == expected);

    constexpr dawn::core::provenance::Sha256Digest emptyExpected{
...
void current_identity_preserves_destiny_fields_and_hashes_producer() {
    ...
    CHECK(!dawn::core::provenance::capture(GetModuleHandleW(L"kernel32.dll")));
    CHECK(dawn::core::provenance::capture(GetModuleHandleW(nullptr)));
    ...
    std::array<char, dawn::core::log::kLineCapacity> line{};
    std::size_t length = 0;
    CHECK(dawn::core::provenance::format_startup_record(line, length));
...
void producer_digest_mismatches_are_independent(TemporaryCacheFile& file,
    ...
    CHECK(file.write_value(header));
    expect_status("different Dawn source digest", fixture.load(file), LoadStatus::stale);
    ...
    CHECK(file.write_value(header));
    expect_status("different Dawn image digest", fixture.load(file), LoadStatus::stale);
    CHECK(fixture.diagnostic.mismatches == cache::Mismatch::producerImage);

    header.producerSourceSha256[0] = std::byte{0xA5};
    CHECK(file.write_value(header));
    expect_status("different Dawn source and image digest", fixture.load(file), LoadStatus::stale);
