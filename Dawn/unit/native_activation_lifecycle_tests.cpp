...
using namespace dawn::client::hooks::activity_lifecycle;
...
void call_gate_covers_publication_wait_and_nested_in_flight_detach() {
    ...
    dawn::client::hooking::CallGate gate{};
    std::atomic<FakeOriginal> original{};
    std::latch entered{1};
    g_fakeOriginalCalled.store(false, std::memory_order_release);

    std::thread waiting([&]() noexcept {
        dawn::client::hooking::CallGate::Scope call(gate);
        entered.count_down();
        const FakeOriginal forwarded = dawn::client::hooking::await_original(original);
        forwarded();
    });
    entered.wait();
    CHECK(gate.active_calls() == 1U);
    CHECK(!g_fakeOriginalCalled.load(std::memory_order_acquire));
    dawn::client::hooking::publish_original(original, &fake_original);
    waiting.join();
    CHECK(g_fakeOriginalCalled.load(std::memory_order_acquire));
    CHECK(gate.idle());
    CHECK(!gate.accepting());

    std::array<dawn::client::hooking::CallGate, kNativeActivationHookCount> gates{};
    NativeActivationBatchState batch{};
    ...
    CHECK(batch.quiesce());
    {
        dawn::client::hooking::CallGate::Scope outer(gates[2]);
        CHECK(gates[2].active_calls() == 1U);
        {
            dawn::client::hooking::CallGate::Scope nested(gates[0]);
