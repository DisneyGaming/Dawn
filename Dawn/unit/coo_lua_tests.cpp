...
void omega_lua_documents() {
    std::string error;
    auto baseline=s::MissionDocument::parse_lua(read("Dawn/scripts/omega.lua"),s::omega_profile(),error);
    if(!baseline) { std::fprintf(stderr,"%s\n",error.c_str()); }CHECK(baseline);
    auto authored=s::MissionDocument::read("Dawn/scripts/omega.lua",s::omega_profile(),error);
    ...
    auto adapter=s::Document::read("Dawn/scripts/omega.lua",error);CHECK(adapter);
...
int main() {
    omega_lua_documents();
    const auto source=read("Dawn/scripts/deadly_trial.lua");std::string error;
    auto lua=s::MissionDocument::read("Dawn/scripts/deadly_trial.lua",trial::kProfile,error);
    ...
    CHECK(!s::MissionDocument::read("Dawn/scripts/absent.lua",trial::kProfile,error));CHECK(error.find("absent.lua")!=std::string::npos);
