...
int main() {
    ...
    std::ifstream input("Dawn/unit/fixtures/native_policy_alternate.json",std::ios::binary);CHECK(input.good());
    ...
    auto fromFile=s::MissionDocument::read_native_policy("Dawn/unit/fixtures/native_policy_alternate.json",profile,error);CHECK(fromFile);
    CHECK(fromFile->same_structure(*doc));
    CHECK(!s::MissionDocument::read("Dawn/unit/fixtures/native_policy_alternate.json",profile,error));
    CHECK(error.find(".lua extension")!=std::string::npos);
    CHECK(!s::MissionDocument::read_native_policy("Dawn/unit/fixtures/mission_script_alternate.lua",profile,error));
