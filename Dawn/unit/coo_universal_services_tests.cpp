...
namespace n=dawn::client::hooks::bootflow::coo_native;
unsigned checks{};
...
void objectives() {
    ...
    auto writer=dawn::middleware::encoding::bits::Writer::measuring();CHECK(c::native_presentation::objective(writer,objective.state()));CHECK(writer.bit_count()==4802);
    for(bool active:{true,false}) {
        objective.set(12,second);if(!active) { objective.clear(); }
        std::array<std::byte,1024> bytes{};dawn::middleware::encoding::bits::Writer encoded(bytes);
...
void captured_components() {
    ...
    std::ifstream file("Dawn/unit/fixtures/gateway_controller_components.bin",std::ios::binary);CHECK(file.good());
...
void alternate_script() {
    ...
    std::ifstream input("Dawn/unit/fixtures/mission_script_universal.lua");CHECK(input.good());std::string text((std::istreambuf_iterator<char>(input)),{}),error;
