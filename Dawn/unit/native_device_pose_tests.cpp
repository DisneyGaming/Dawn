...
#include <cstdio>
#include <limits>
namespace pose=dawn::server::runtime::activity::mission_device_pose;
namespace wire=dawn::middleware::bap::activity_message::native::generic_device;
...
int main() {
    ...
    dawn::server::runtime::activity::mission_capture::Publication capture;
    check(pose::desire(capture.pose,.2F),"capture pose initialized independently");
    const auto saved=capture.pose.state;
    check(dawn::server::runtime::activity::mission_capture::update(capture,1,true,false,100,20)
        && capture.pose.state==saved,"capture start cannot erase pose revision history");
    check(dawn::server::runtime::activity::mission_capture::update(capture,2,false,false,100,25)
    ...
    namespace object=dawn::state::activity::coo::native_device;
