#pragma once
#include "../../../src/client/hooks/bootflow/deep_storage_lens_damage.h"
#include <array>
#include <cstring>
namespace deep_lens_damage_fixture {
namespace damage=dawn::client::hooks::bootflow::deep_storage_lens_damage;
namespace identity=dawn::client::hooks::bootflow::native_box_identity;
...
struct Memory {
    ...
    Memory() {
        ...
        put(source,std::uint32_t{0x80B568A6});put(source+4,std::uint32_t{0x80809928});put(source+8,std::uint64_t{0x4C8});
...
inline void run(void (*check)(bool,const char*)) {
    ...
    ds::LensRequest request{};request.owner={90,1};request.enabled=true;request.objectGeneration=2;
    ...
    check(!damage::current(m,request,candidate,sample),"Gateway source cannot become a Deep Storage lens");m.put(Memory::source,std::uint32_t{0x80B568A6});
