...
using dawn::core::settings::Settings;
using dawn::core::settings::experiments::Omega;
...
bool parse_document(std::string_view document, Settings& output) noexcept {
    return dawn::core::settings::parse(document, output);
}

void all_off_defaults_are_stable() {
    g_context = "all-off defaults";
    CHECK(dawn::core::settings::kSettingsVersion == 6U);

    const Settings fixedDefaults = dawn::core::settings::defaults();
...
int main() {
    ...
    CHECK(dawn::core::settings::parser::g_unexpectedParserBranchCalls == 0U);
