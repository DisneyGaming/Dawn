...
namespace a=dawn::server::runtime::activity;
...
int main(int argc,char** argv) {
    ...
    const auto doc=load("Dawn/scripts/infinite_abyss.json",hf::kProfile);
    ...
    std::ifstream mercuryInput("Dawn/scripts/mercury_freeroam.json",std::ios::binary);CHECK(mercuryInput.good());
    ...
    a::capture_bridge::release(owner);
    stack_smoke("Dawn/scripts/city_tower_social_d2.json",a::city_tower_social_d2::kActivity);
    stack_smoke("Dawn/scripts/mercury_freeroam.json",a::mercury::kActivity);
    stack_smoke("Dawn/scripts/infinite_abyss.json",hf::kActivity);
