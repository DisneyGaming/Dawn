...
Dawn keeps the native Grandmaster activity power at 1100 and publishes the
...
check. Dawn does not draw a substitute sword icon.
...
object rather than 36 safe skull IDs. Dawn therefore leaves the opaque native
...
substitution. Dawn now freezes variant 5 in the strike frame only for exact
...
profile fields have stable widths but no recovered semantics, so Dawn leaves
...
idempotent. Account inventory is currently process-local across all Dawn item
