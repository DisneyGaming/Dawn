...
bool initialize(void* module, std::uint64_t configuredEquipmentHash) noexcept {
    ...
    const cache::LoadStatus status =
        cache::load(persistenceState.cachePath.chars.data(),
                    ...
                    counts,
                    &diagnostic);
    std::array<char, core::log::kLineCapacity> cacheLine{};
    std::size_t cacheLineLength = 0;
    if (!cache::format_outcome_record(
            persistenceState.buildIdentity, diagnostic, cacheLine, cacheLineLength)) {
        runtime::persistence::clear_locked(persistenceState);
        ReleaseSRWLockExclusive(&persistenceState.lock);
        return false;
    }
    core::log::write_startup_record(
        core::log::Channel::state,
        std::string_view(cacheLine.data(), cacheLineLength));
    ...
    if (status == cache::LoadStatus::stale
        || (status == cache::LoadStatus::invalid
            && diagnostic.reason == cache::LoadReason::magic)) {
        // A migrated cache can carry an obsolete producer signature. Its payload is never
        // published; rebuild from packages and replace it only after every domain is complete.
        // Other failures (including unreadable files) retain their existing failure behavior.
    ...
    scenarios::export_omega_inventory(domains.scenarios, domains.rosterGroups);
    scenarios::export_towerfall_cue_manifest(domains.scenarios, domains.rosterGroups);
    runtime::persistence::release_scratch_locked(persistenceState);
    ReleaseSRWLockExclusive(&persistenceState.lock);
    return true;
}
