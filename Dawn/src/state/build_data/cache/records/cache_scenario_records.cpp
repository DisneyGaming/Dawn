...
[[nodiscard]] bool storable(const scenarios::Definition& value) noexcept {
    if (value.nameLength == 0 || value.nameLength > scenarios::kNameCapacity
    ...
    for (const std::uint8_t count : value.authoredGroupCounts) {
        if (count > scenarios::kDestinationAuthoredGroupCapacity) {
            return false;
        }
    }
    return true;
...
bool encode(const scenarios::Definition& value, ScenarioRecord& record) noexcept {
    ...
    std::copy(value.authoredGroupCounts.begin(),
              value.authoredGroupCounts.end(),
              record.authoredGroupCounts.begin());
    std::copy(value.authoredGroups.begin(),
              value.authoredGroups.end(),
              record.authoredGroups.begin());
...
bool decode(const ScenarioRecord& record, scenarios::Definition& value) noexcept {
    ...
    for (const std::uint8_t count : record.authoredGroupCounts) {
        if (count > scenarios::kDestinationAuthoredGroupCapacity) {
            return false;
        }
    }
    ...
    std::copy(record.authoredGroupCounts.begin(),
              record.authoredGroupCounts.end(),
              value.authoredGroupCounts.begin());
    std::copy(record.authoredGroups.begin(),
              record.authoredGroups.end(),
              value.authoredGroups.begin());
...
bool encode(const scenarios::RosterGroup& value, RosterGroupRecord& record) noexcept {
    ...
    std::copy(value.descriptorTags.begin(),
              value.descriptorTags.end(),
              record.descriptorTags.begin());
    std::copy(value.descriptorOffsets.begin(),
              value.descriptorOffsets.end(),
              record.descriptorOffsets.begin());
    std::copy(value.componentClasses.begin(),
              value.componentClasses.end(),
              record.componentClasses.begin());
    std::copy(value.senseSchemas.begin(), value.senseSchemas.end(), record.senseSchemas.begin());
    std::copy(value.authSchemas.begin(), value.authSchemas.end(), record.authSchemas.begin());
    return true;
}
...
bool decode(const RosterGroupRecord& record, scenarios::RosterGroup& value) noexcept {
    ...
    std::copy(record.descriptorTags.begin(),
              record.descriptorTags.end(),
              value.descriptorTags.begin());
    std::copy(record.descriptorOffsets.begin(),
              record.descriptorOffsets.end(),
              value.descriptorOffsets.begin());
    std::copy(record.componentClasses.begin(),
              record.componentClasses.end(),
              value.componentClasses.begin());
    std::copy(record.senseSchemas.begin(), record.senseSchemas.end(), value.senseSchemas.begin());
    std::copy(record.authSchemas.begin(), record.authSchemas.end(), value.authSchemas.begin());
    return true;
}
