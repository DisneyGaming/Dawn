...
namespace dawn::state::activity::vendors {
...
// Exact native assets owned by the vendor lifetime. Other NPCs in the same
// destination retain their ordinary patrol policy.
inline constexpr bool owns(std::uint32_t scenario,std::uint32_t key,
    std::uint8_t type,std::uint16_t slot) noexcept {
    for(const auto& group:groups(scenario)) if(group.key==key)
        for(const auto& binding:group.slots)
            if(binding.asset.type==type && binding.asset.slot==slot) return true;
    return false;
}
