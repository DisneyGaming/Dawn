...
// scenario 80F54AE7 through the Dawn activity SDK export, with the cohort split, gating and
// checkpoint choices carried across from the mission logic that already runs this encounter
// (dawn-strike-missions/strike_pact/{populations,route}.lua). Registry keys are the accepted
// Dawn host's published roster, so a slot id's object tag selects the group it belongs to.
#pragma once
#include <array>
#include <cstdint>
#include <string_view>
