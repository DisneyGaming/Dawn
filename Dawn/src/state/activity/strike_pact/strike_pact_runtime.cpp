...
bool load(bool campaign) noexcept {
    ...
    std::call_once(once[campaign?1:0],[&] {
        ...
        try {
            ...
            else { document=coo::script::MissionDocument::read(std::filesystem::path(path.data()).parent_path()/L"Dawn"/L"scripts"/(campaign?L"mission_pact.lua":L"strike_pact.lua"),kProfile,error); }
        ...
        if(document) {
            std::snprintf(line.data(),line.size(),"ev=coo_script mission=%s result=loaded format=lua fnv1a64=%016llX path=Dawn/scripts/%s.lua scope=mission reload=next_process",
