...
bool load() noexcept {
    static std::once_flag once;std::call_once(once,[] {
        ...
        try {
            ...
            else { document=coo::script::MissionDocument::read(std::filesystem::path(path.data()).parent_path()/L"Dawn"/L"scripts"/L"deadly_trial.lua",kProfile,error); }
        ...
        if(document) { std::snprintf(line.data(),line.size(),"ev=coo_script mission=deadly_trial result=loaded format=lua script=Dawn/scripts/deadly_trial.lua fnv1a64=%016llX reload=next_process",static_cast<unsigned long long>(document->fingerprint())); }
