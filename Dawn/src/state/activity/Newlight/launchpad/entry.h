...
inline constexpr coo::script::Profile kEntryProfile{
    "launchpad.native.v1", "otherMissions", coo::Schema::otherMissions,
...
inline bool valid_document(const coo::script::Views& views) noexcept {
    ...
    if (!views.valid || views.missionId != "launchpad" || views.profileId != kEntryProfile.id
...
// A small composition adapter. The process-owned Lua document stays separate from
// the native run state and its mission completion wait.
class Entry final {
public:
