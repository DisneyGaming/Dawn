...
namespace dawn::state::activity::newlight::launchpad::tower {
namespace native=omega_ending_transit;
inline constexpr std::int16_t kApproachActivity=2,kTowerActivity=20;
...
public:
    ...
    std::int16_t wanted() const noexcept {return s.phase==Phase::approachRequested?kApproachActivity:s.phase==Phase::towerRequested?kTowerActivity:-1;}
    bool queued(std::uint64_t run,std::int16_t index,std::uint64_t now) noexcept {
        if(run!=s.run || index!=wanted() || index<0) {return false;}
        s.phase=index==kApproachActivity?Phase::approachLoading:Phase::towerLoading;s.deadline=now+180000;return true;
    }
    void selected(std::uint64_t run,std::int16_t index,std::uint32_t scenario,std::uint64_t now) noexcept {
        ...
        if((s.phase==Phase::approachLoading && index==1 && scenario==kScenario)
            || (s.phase==Phase::towerLoading && index==kApproachActivity && scenario==kApproachScenario)) {return;}
        if(s.phase==Phase::approachLoading && index==kApproachActivity && scenario==kApproachScenario) {
            s.run=run;s.phase=Phase::preparing;s.deadline=now+90000;s.revision=1;
        } else if(s.phase==Phase::towerLoading && index==kTowerActivity && scenario==kTowerScenario) {
