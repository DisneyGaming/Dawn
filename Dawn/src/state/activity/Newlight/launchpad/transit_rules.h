...
namespace dawn::state::activity::newlight::launchpad::transit {
...
struct Departure {
    Scope scope{};std::uint8_t transition{};
    bool operator==(const Departure&) const = default;
};
constexpr native::Target destination(std::uint8_t route) noexcept {
    // Each cinematic must use a spawn set from its own map bubble. The Breach
    // default (2EA8FB98) does not exist in Dock13 and stalls its teleport arrival.
    return route>=1 && route<=3?native::Target{cinematics::kMovies[route-1].region,route==1?0xEAEC2335U:0x573574C2U}
        :route==4?native::Target{24,0xEAEC2335U}:native::Target{};
}
...
template<class Leg> constexpr bool dock_arrived(const Leg& current,const Leg& pending) noexcept {
    // Being physically in Dock13 does not finish the public-session swap. A C9
    // here preempts that cleanup; wait for native retirement of the outgoing leg.
    return current.present && current.sliceSetIndex==16 && pending.present && pending.sliceSetIndex==-1;
}
template<class Leg> constexpr bool divide_retained(const Leg& current,const Leg& pending) noexcept {
    return current.present && current.sliceSetIndex==16 && pending.present && pending.sliceSetIndex==8;
}
constexpr bool departure_allowed(const cinematics::State& movie,std::uint8_t section) noexcept {
    return section==4 && (movie.phase==cinematics::Phase::gameplay
        || (movie.movie==1 && movie.phase==cinematics::Phase::preparing));
}
template<class Membership,class Wire> void echo_regions(const Membership& membership,Wire& wire) noexcept {
    const auto copy=[](const auto& v,auto& out) {
        out={v.sliceSetIndex,v.sliceSetHash,v.regionIndex,v.publicState,v.auxState,v.present};
    };
    copy(membership.currentLeg,wire.currentLeg);copy(membership.pendingLeg,wire.pendingLeg);
}
