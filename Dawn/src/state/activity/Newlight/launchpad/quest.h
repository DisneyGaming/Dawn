...
namespace dawn::state::activity::newlight::launchpad::quest {
...
inline bool escaped(const CharacterState& character) noexcept {
    std::int32_t value{};
    return ::dawn::state::vendors::lookup(character.vendorUnlocks.flags,1041,value) && value==2;
}
inline bool record_escape(CharacterState& character) noexcept {
    auto flags=character.vendorUnlocks.flags;
    if(!::dawn::state::vendors::store(flags,753,1)
        || !::dawn::state::vendors::store(flags,1041,2)) {return false;}
...
template<class CharacterObject> inline void project(const CharacterState& character,CharacterObject& object) noexcept {
    ...
    // Preserve the vendor overrides already projected into the same native banks.
    const auto flag=[&](std::int16_t slot,std::uint8_t value) {
        std::size_t at{};
        for(;at<object.unlockFlagCount;++at) if(object.unlockFlags[at].slot==slot) break;
        if(at>=object.unlockFlags.size()) return;
        if(at==object.unlockFlagCount) ++object.unlockFlagCount;
        object.unlockFlags[at]={slot,value,0};
    };
    const auto numeric=[&](std::int16_t slot,std::int32_t value) {
        std::size_t at{};
        for(;at<object.unlockValueCount;++at) if(object.unlockValues[at].slot==slot) break;
        if(at>=object.unlockValues.size()) return;
        if(at==object.unlockValueCount) ++object.unlockValueCount;
        object.unlockValues[at]={slot,0,value};
    };
    flag(20483,2);
    for(int i=0;i<3;++i) flag(static_cast<std::int16_t>(20484+i),current==i+2?2:0);
    numeric(12740,kTracking[current]);
    for(int i=0;i<5;++i) numeric(static_cast<std::int16_t>(12741+i),current>i?1:0);
}
}
