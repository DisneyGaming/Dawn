...
bool load(bool campaign) noexcept {
    ...
    std::call_once(once[campaign?1:0],[&] {
        ...
        try {
            ...
            else { document=coo::script::MissionDocument::read(std::filesystem::path(path.data()).parent_path()/L"Dawn"/L"scripts"/(campaign?L"mission_bond.lua":L"strike_bond.lua"),kProfile,error); }
        ...
        if(document) { std::snprintf(line.data(),line.size(),"ev=coo_script mission=%s result=loaded format=lua fnv1a64=%016llX path=Dawn/scripts/%s.lua reload=next_process",campaign?"mission_bond":"strike_bond",static_cast<unsigned long long>(document->fingerprint()),campaign?"mission_bond":"strike_bond"); }
