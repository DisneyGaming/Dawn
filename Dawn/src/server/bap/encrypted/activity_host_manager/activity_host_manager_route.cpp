...
prepare_allocation(const request_selection::ActivityManagerSelectionResult& parsed,
                   state::activity::ActivityInstanceKey replaces,
    ...
    if (!hasCopy || !source.hasPackageName) {
        ...
        if (state::activity::forced::apply(forced)) {
            ...
            return state::activity::prepare_session(forced, replaces, sessionId, allocation);
        }
        return state::activity::prepare_session(replaces, sessionId, allocation);
    }
    ...
    const auto hauntedArrival =
        ::dawn::server::runtime::activity::haunted_forest::apply_initial_arrival(destination);
    if (hauntedArrival == ::dawn::server::runtime::activity::initial_arrival::Result::applied) {
    ...
    // Omega forest arrival: the gate entrance armed a one-shot override, and the next
    // mission_scot launch arrives inside the forest bubble (region 88) with the forest
    // spawn point, through the proven initial-slice-set path. Applied after forced::apply
    // because forced overwrites the selection's override fields unconditionally.
    {
        const std::string_view launchName(
            reinterpret_cast<const char*>(destination.packageName.data()),
            destination.packageNameLength);
        if (launchName == "mission_scot"
            && state::activity::consume_omega_forest_arrival_pending()) {
            // Retail-verified gate arrival: the far side of the opening gate is
            // infinite_forest_d (bubble 11) at spawn set 0xB34BD817 -- the gate tunnel,
            // authored in BOTH the lighthouse and forest_d bubbles (the location popup
            // fires halfway through it, at the region-volume boundary the walked z-leg
            // crossing uses).
            destination.arrivalBubbleOverride = 11;  // region 88, PRV88.88
            destination.hasArrivalBubbleOverride = true;
            // The panel launch forces slice 120, and a forced slice set beats the bubble
            // override in arrival_slice_set, so the forest slice must be forced too.
            destination.sliceSetOverride = 88;
            destination.hasSliceSetOverride = true;
            destination.spawnSetOverride = 0xB34BD817U;  // gate-tunnel spawn set
            destination.hasSpawnSetOverride = true;
            core::log::write(core::log::Channel::server, core::log::Level::info,
                             "ev=activity stage=omega_forest_arrival result=applied "
                             "bubble=11 slice=88 spawn_set=0xB34BD817");
        }
    }
    return state::activity::prepare_session(destination, replaces, sessionId, allocation);
}
...
bool encode_response(std::span<const std::byte> requestBody,
    ...
    if (!prepare_allocation(selection, replaces, sessionId, prepared)) {
        core::log::write(core::log::Channel::server,
                         core::log::Level::warn,
    ...
    // Correlate the exact response body with the pinned client's already-installed bit readers.
    // This is observation-only: the encoder output and the deferred State transaction are not
    // changed by the probe.
    client::hooks::bootflow::arm_activity_host_manager_response_decode_probe(
        sessionId, output.data(), written);
