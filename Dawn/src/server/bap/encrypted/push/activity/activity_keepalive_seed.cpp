...
bool seed_identity(state::activity::ActivityInstanceKey activity,
                   std::uint64_t memberKey,
                   std::uint64_t characterSoid) noexcept {
    if (!static_cast<bool>(activity) || memberKey == 0) {
        return false;
    }
    ...
    const bool seeded = state::activity::membership::prepare_identity(activity, identity, mutation)
...
bool seed_transition_token(state::activity::ActivityInstanceKey activity) noexcept {
    ...
    state::activity::membership::PendingMutation mutation{};
    const bool seeded =
        state::activity::membership::prepare_authoritative(activity, update, mutation)
        && state::activity::membership::commit(mutation);
    SecureZeroMemory(&mutation, sizeof mutation);
    return seeded;
}
