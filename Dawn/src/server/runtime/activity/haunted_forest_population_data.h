...
namespace population = dawn::server::runtime::activity::population;
namespace codec = dawn::middleware::bap::activity_message::native::combatant_source;
...
inline constexpr std::array<population::Capability,kPopulationCount> kPopulationCapabilities=[] {
    ...
    for(const auto& boss:kBosses) {
        output[cursor++]=population::Capability{.registry=kPopulationRegistry,
            .slot=boss.squad.slot,.rule=kBossRuleSlot,.tactical=kBossTactical,
            .allowCycles=true,.namedMember=boss.combatant.slot,.categories=1};
    }
    const auto append=[&](const auto& sources) constexpr {
        for(const auto& source:sources) {
            output[cursor++]=population::Capability{.registry=kPopulationRegistry,
                .slot=source.source.slot,.rule=kAddRuleSlot,.tactical=kAddTactical,
                .allowCycles=true,.namedMember=population::kNoNamedMember,.categories=1};
        }
    };
