...
bool build(const JoinEndpoint& endpoint, std::array<std::byte, kDescriptorSize>& output) noexcept {
    ...
    // The captured direct-path descriptor leaves the join key clear and carries the online
    // session identity in the low half of the final 18-byte field.
    write_memory_order(candidate,
                       kSessionOffset,
                       endpoint.onlineSessionId,
                       sizeof(std::uint64_t));
    output = candidate;
    return true;
}
