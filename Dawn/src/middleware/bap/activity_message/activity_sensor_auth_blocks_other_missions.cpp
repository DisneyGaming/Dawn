...
bool legacy_pad_bits(bits::Writer& writer, std::size_t count) noexcept {
    bool encoded = true;
...
bool legacy_write_bubble_block(bits::Writer& writer, const Grant& grant) noexcept {
    bool encoded = true;
...
bool legacy_write_roster_delta(bits::Writer& writer,
                        const Roster& roster,
                        std::uint8_t stateSequence) noexcept {
...
bool legacy_write_object_block(bits::Writer& writer,
    ...
    if (encoded && emitAuth) {
        ...
        if (encoded && body > 0) {
            encoded = legacy_write_auth_body(
