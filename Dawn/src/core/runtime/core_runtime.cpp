...
#include "../provenance/build_provenance.h"
#include "../settings/settings.h"
#include "../ui/modules/hud/hud.h"
#include "../ui/modules/logs/logs.h"
#include "../ui/modules/loadout/loadout.h"
#include "../ui/layout/credits/dawn_credits_badge.h"
...
bool initialize(void* module) noexcept {
    ...
    if (!log::initialize(module, settings::get().logging)) {
        ...
        std::array<char, log::kLineCapacity> identityLine{};
        std::size_t identityLength = 0;
        if (!provenance::capture(static_cast<HMODULE>(module))
            || !provenance::format_startup_record(identityLine, identityLength)) {
            stage = "provenance";
        } else {
            log::write_startup_record(
                log::Channel::core,
                std::string_view(identityLine.data(), identityLength));
        }
        if (stage != nullptr) {
            // The named provenance stage is reported by the shared failure path below.
            ...
            stage = "ui_logs";
        } else if (!ui::modules::loadout::initialize()) {
            stage = "ui_loadout";
        } else if (!ui::layout::credits::initialize()) {
            stage = "ui_credits";
