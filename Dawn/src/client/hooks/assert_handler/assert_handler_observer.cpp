...
/** Bounded native stack retained for the first occurrence of each distinct assertion. */
constexpr std::size_t kStackFrameCapacity = 32;
...
/** Records module-relative return addresses without symbol lookup or heap allocation. */
void report_stack(std::uint32_t seen) noexcept {
    std::array<void*, kStackFrameCapacity> frames{};
    const USHORT count = RtlCaptureStackBackTrace(
        2, static_cast<ULONG>(frames.size()), frames.data(), nullptr);
    const auto mainImage = reinterpret_cast<std::uintptr_t>(GetModuleHandleW(nullptr));
    std::array<char, core::log::kLineCapacity> line{};
    int written = std::snprintf(line.data(),
                                line.size(),
                                "ev=assert stage=stack n=%u thread=%lu frames=",
                                seen,
    ...
    for (USHORT index = 0; index < count && used + 48U < line.size(); ++index) {
        void* module = nullptr;
        (void)RtlPcToFileHeader(frames[index], &module);
        ...
        const int appended = moduleBase == mainImage
                                 ? std::snprintf(line.data() + used,
                                                 line.size() - used,
                                                 "%sexe+%llX",
                                                 index == 0 ? "" : ",",
                                                 static_cast<unsigned long long>(address - mainImage))
                                 : std::snprintf(line.data() + used,
                                                 line.size() - used,
                                                 "%s%p+%llX",
                                                 index == 0 ? "" : ",",
                                                 ...
                                                 static_cast<unsigned long long>(
                                                     moduleBase != 0U ? address - moduleBase
                                                                      : address));
        if (appended <= 0) {
            break;
        }
        used += (std::min)(static_cast<std::size_t>(appended), line.size() - used - 1U);
    }
    line[used] = '\0';
...
void __cdecl handler_body(int code, const char* format, ...) noexcept {
    ...
    inside = true;
    network::lifecycle::sensor_state_heap::notify_assert(text.data());
