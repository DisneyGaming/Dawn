...
bool reloaded(const Entry& entry,gateway_native::Weak& source,const char*& reason) noexcept {
    ...
    const state::activity::coo::native_catalog::AssetBinding* binding{};
    using Binding=state::activity::coo::native_catalog::AssetBinding;
    for(auto assets:{std::span<const Binding>(state::activity::vendors::presentation::kAssets),std::span<const Binding>(state::activity::vendors::kAssets)})
