...
bool shutdown() noexcept {
    ...
    // Close every retail-triggered bootflow entry, then drain and detach that producer before any
    // bootflow consumer can be removed. A retained producer aborts shutdown with all dependencies
    // still published for a safe retry.
    hooks::bootflow::quiesce();
    hooks::retail_log::quiesce();
    if (!hooks::retail_log::uninstall()) {
        core::log::write(core::log::Channel::client,
                         core::log::Level::error,
                         "ev=shutdown stage=retail_log result=retained");
        ReleaseSRWLockExclusive(&runtime::g_lock);
        return false;
    }
    ...
    if (!hooks::bootflow::uninstall()) {
        core::log::write(core::log::Channel::client,
                         core::log::Level::error,
                         "ev=shutdown stage=bootflow_hooks result=deferred");
        ReleaseSRWLockExclusive(&runtime::g_lock);
        return false;
    }
