...
namespace dawn::client::ui::mission_launch::detail {
/** Shared contrast for every Activity Launcher page; restores the surrounding overlay palette on exit. */
struct Style final {
    Style() noexcept {
    ...
    ~Style() noexcept { ImGui::PopStyleVar(2); ImGui::PopStyleColor(14); }
...
[[nodiscard]] inline bool launch_button(bool busy, bool disabled, float height) noexcept {
    ImGui::PushStyleColor(ImGuiCol_Button, disabled ? ImVec4{0.19F, 0.24F, 0.30F, 1} : ImVec4{0.97F, 0.51F, 0.25F, 1});
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, disabled ? ImVec4{0.91F, 0.94F, 0.98F, 1} : ImVec4{0.035F, 0.045F, 0.065F, 1});
    ImGui::BeginDisabled(disabled);
    const bool clicked = ImGui::Button(busy ? "Launching..." : "Launch activity", {-1.0F, height});
    ImGui::EndDisabled(); ImGui::PopStyleColor(4);
    return clicked;
}
} // namespace dawn::client::ui::mission_launch::detail
