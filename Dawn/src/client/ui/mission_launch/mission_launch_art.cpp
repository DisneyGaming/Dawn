...
namespace dawn::client::ui::mission_launch::art {
namespace {
...
void prepare(ID3D11Device* device) noexcept {
    ...
    if (device != g_device) { release(); g_device = device; g_device->AddRef(); }
    const auto images = state::build_data::activities::artwork();
    if (g_attempted || images.empty()) { return; }
    g_attempted = true;
    for (std::size_t i = 0; i < images.size(); ++i) {
        const auto& image = images[i];
        if (image.pixels.empty()) { continue; }
        ...
        data.pSysMem = image.pixels.data(); data.SysMemPitch = static_cast<UINT>(image.width) * 4;
...
DisplaySize display_size(state::build_data::activities::Icon icon, float extent, float framebufferScale) noexcept {
    namespace catalog = state::build_data::activities;
    if (!std::isfinite(extent) || extent <= 0.0F) { return {}; }
    if (!std::isfinite(framebufferScale) || framebufferScale <= 0.0F) { framebufferScale = 1.0F; }
    const auto images = catalog::artwork();
    const auto index = static_cast<std::size_t>(icon);
    if (index >= images.size() || images[index].width == 0 || images[index].height == 0) { return {}; }
