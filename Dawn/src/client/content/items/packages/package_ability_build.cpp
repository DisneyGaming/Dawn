...
summary_entries(const domain::Selection& selection) noexcept {
    // Melee claims before super: the bottom tree's super (entry 20) shares a group with its melee
    // (21), and a super claiming first left the melee and its tree perks inactive.
    return {kSprintEntry,
 ...
 * @param kind Keeps the entry's declared kind, or inherits the first kind in its link chain.
 * @return True when the chain reaches a destination inside the bucket range.
 */
[[nodiscard]] bool selector_destination(const Walk& walk,
    ...
    for (std::size_t step = 0; step < kSelectorChainLimit; ++step) {
        ...
        // The entry's own kind wins. A linked record's kind only fills an entry that declares none,
        // which is Arcstrider's and Sentinel's bare tree-3 super; overriding a declared kind files
        // every ability hash under the wrong bucket and empties the character's abilities.
        if (kind == pool::kEmptyByte && record.kind != pool::kEmptyByte) {
            kind = record.kind;
        }
...
[[nodiscard]] bool assign_kinds(const Walk& walk, domain::Definition& output) noexcept {
    for (const std::uint8_t entryIndex : walk.selected) {
        ...
        std::array<pool::PoolRecord, pool::kPoolRecordCapacity> records{};
        std::uint8_t bucket = pool::kEmptyByte;
        const std::size_t count = records_of(walk, walk.entries[entryIndex], 0, records);
        // Each refusal names the entry and the step, so a missing ability row can be attributed.
        // `first` is the record count, `second` packs kind << 16 | bucket << 8 | destination.
        const auto refuse = [&](const char* stage) noexcept {
            const std::size_t packed = (static_cast<std::size_t>(count != 0 ? records[0].kind : 0xFF) << 16U)
                                       | (static_cast<std::size_t>(bucket) << 8U)
                                       | (count != 0 ? records[0].destination : 0xFFU);
            report_ability_failure(stage, entryIndex, count, packed);
            return false;
        };
        if (count == 0) {
            return refuse("kinds_no_records");
        }
        std::uint8_t kind = records[0].kind;
        if (!selector_destination(walk, entryIndex, bucket, kind)) {
            return refuse("kinds_no_destination");
        }
        // Arcstrider's and Sentinel's tree-3 super is a bare link: its own record carries no kind,
        // so the kind is the one the chain lands on.
        if (kind == pool::kEmptyByte) {
            return refuse("kinds_no_kind");
        }
        if (output.buckets[bucket].kind != domain::kEmptyBucketKind) {
            return refuse("kinds_bucket_taken");
        }
        output.buckets[bucket].kind = kind;
    }
    return true;
}
