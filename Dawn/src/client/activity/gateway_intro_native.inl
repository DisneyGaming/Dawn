...
void gateway_depart(std::uintptr_t base,std::int32_t step) noexcept {
    ...
    if(step!=38 || !gateway_nonce(session,nonce) || nonce!=g_gatewayExit.nonce
        || !read(session+0x182C0+0x148+4,index) || index!=intro::kMission
    ...
    core::log::write(core::log::Channel::client,core::log::Level::info,"ev=gateway_intro stage=mission_departure");
    leave(28,309);
}
