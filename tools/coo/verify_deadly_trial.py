...
try:
 for name in projects:
  results.append(verify.build(Path('Dawn/unit/'+name+'.vcxproj').resolve(),'Release'))
 results.append(verify.build(Path('Dawn/unit/deadly_trial_tests.vcxproj').resolve(),'Debug'))
 (verify.OUT/'tests.json').write_text(json.dumps(results,indent=2)+'\n')
 result=verify.build(Path('Dawn/Dawn.vcxproj').resolve(),'Release')
