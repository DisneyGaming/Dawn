"""Package a complete Lua validation run without installing or launching Destiny."""
import argparse
...
ROOT = verify.ROOT
SCRIPTS = ('omega.lua', 'deadly_trial.lua', 'gateway.lua', 'beyond_infinity.lua', 'deep_storage.lua', 'strike_pact.lua')
...
def main():
    ...
    expected = {(name, conf) for name in (*verify_lua.TESTS, 'Sunrise') for conf in ('Debug', 'Release')}
    require(len(results) == len(expected) and {(r['project'], r['configuration']) for r in results} == expected,
            'Packaging requires every Debug and Release suite and both DLL builds.')
    ...
    manifest = {
        ...
        'buildsAndTests': len(results), 'compilerWarnings': 0,
