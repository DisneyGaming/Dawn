...
/**
 * Selects the redirect address for one destination port. Gameplay owns a second loopback address
 * so the Client and embedded host may bind the same UDP port without receiving their own traffic.
 */
[[nodiscard]] std::array<unsigned char, 4>
redirect_octets_for_port(std::uint16_t port) noexcept {
    const core::settings::server::gameplay::Settings& gameplay =
        core::settings::get().server.gameplay;
    if (gameplay.topology != core::settings::server::gameplay::Topology::disabled
        && port == gameplay.port) {
        return gameplay.transportAddress;
    }
...
bool is_redirect_target(const sockaddr* address, int addressLength) noexcept {
    ...
    const std::array<unsigned char, 4> octets =
        redirect_octets_for_port(ntohs(endpoint.sin_port));
...
bool redirect_ipv4(const sockaddr* address, int addressLength, sockaddr_in& redirected) noexcept {
    ...
    const std::array<unsigned char, 4> octets =
        redirect_octets_for_port(ntohs(redirected.sin_port));
