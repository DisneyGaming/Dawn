...
constexpr std::string_view kActivitySessionText = "sunrise-red-war-opening";
    ...
    if (openingSelected && !g_sessionReported.exchange(true, std::memory_order_relaxed)) {
        core::log::write(core::log::Channel::client,
                         sessionPublished ? core::log::Level::info
                                          : core::log::Level::warn,
                         sessionPublished
                             ? "ev=bootflow stage=activity_host_session result=published id=sunrise-red-war-opening reason=membership_ready"
