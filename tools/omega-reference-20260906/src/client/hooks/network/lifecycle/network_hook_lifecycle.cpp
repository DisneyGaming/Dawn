...
bool install_game() noexcept {
    // Text projection is deliberately outside every replacement body.
    lifecycle::sensor_state_heap::drain(64U);
    ...
    if (baseInstalled && content_config::is_installed() && investment::is_installed()) {
        ReleaseSRWLockExclusive(&g_lock);
        (void)lifecycle::sensor_state_heap::install();
        return true;
    }
    ...
    ReleaseSRWLockExclusive(&g_lock);
    if (installed) {
        // This optional default-off observer never controls normal network-hook admission.
        (void)lifecycle::sensor_state_heap::install();
    }
    return installed;
}
...
bool uninstall() noexcept {
    ...
    lifecycle::sensor_state_heap::drain(64U);
    if (!lifecycle::sensor_state_heap::uninstall()) {
        return false;
    }
    lifecycle::sensor_state_heap::drain(2048U);
...
bool has_game_ownership() noexcept {
    ...
    const bool owned = lifecycle::any_installed(lifecycle::kGameSlots)
                       ...
                       || lifecycle::sensor_state_heap::has_ownership();
    ReleaseSRWLockShared(&g_lock);
    return owned;
}
