 ...
 * The implementation retains an exact activity-instance key and reduces it to a scalar session
 * id only at this gameplay compatibility boundary.
 * @return Allocated current session id, or zero while State cannot allocate one yet.
 */
 ...
 * @param regionIndex Region being advertised, or kUnknownRegion.
 * @param sourceActivitySessionId Activity session whose exact current lifetime supplies the
 *        inherited destination. A stale nonzero id fails closed; only zero requests defaults.
 * @param claimedSlot Set when a slot names this region, with or without a session in it yet.
 * @return Allocated current session id, or zero while the slot has none or an exact lifetime is
 *         stale.
 */
[[nodiscard]] std::uint64_t activity_host_session(std::uint64_t groupSessionId,
...
/**
 * Exact source-activity overload used by BAP binding-owned advertisement publishers.
 * State validity is checked before the group table lock is taken; no locks are nested.
 */
