...
[[nodiscard]] const SessionRecord* prepare_base(const ActivityState& state,
    ...
    const ActivityInstanceKey instanceKey = activity::transactions::instance_key(record);
    if (!static_cast<bool>(instanceKey)) {
        return nullptr;
    }
    mutation.instanceKey = instanceKey;
...
/** Captures one exact activity lifetime while the root State read lock is held. */
[[nodiscard]] const SessionRecord* prepare_base(const ActivityState& state,
    ...
    mutation.sessionId = key.sessionId;
    mutation.expectedStateRevision = state.stateRevision;
    mutation.expectedRecordRevision = record.recordRevision;
    mutation.targetSlot = target;
    mutation.prepared = true;
...
bool prepare_grant(ActivityInstanceKey key,
                   std::size_t requestedCount,
                   PendingMutation& mutation) noexcept {
    mutation = {};
    if (!static_cast<bool>(key) || requestedCount == 0) {
        return false;
    }
    ...
    const ActivityState& state = runtime::storage::g_state.activity;
    PendingMutation prepared{};
    const SessionRecord* record = prepare_base(state, key, true, prepared);
    if (record != nullptr) {
...
bool prepare_release(ActivityInstanceKey key,
