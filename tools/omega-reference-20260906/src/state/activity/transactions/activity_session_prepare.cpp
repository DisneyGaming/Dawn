...
[[nodiscard]] bool prepare_locked(const ActivityState& state,
    ...
    prepared.targetSlot = transactions::select_target(state, replaces);
...
/** Prepares one default-destination allocation with an exact optional replacement. */
bool prepare_session(ActivityInstanceKey replaces,
                     std::uint64_t& sessionId,
                     PendingAllocation& allocation) noexcept {
    sessionId = kAbsentSessionId;
    allocation = {};
    ...
    AcquireSRWLockShared(&runtime::storage::g_stateLock);
    const ActivityState& state = runtime::storage::g_state.activity;
    const bool ready = prepare_locked(state,
...
/** Prepares one explicit-destination allocation with an exact optional replacement. */
bool prepare_session(const destination::DestinationSelection& selection,
                     ActivityInstanceKey replaces,
                     std::uint64_t& sessionId,
                     PendingAllocation& allocation) noexcept {
    ...
    const std::uint64_t soidBase = session_soid_base();
    AcquireSRWLockShared(&runtime::storage::g_stateLock);
    const bool ready = prepare_locked(runtime::storage::g_state.activity,
                                      selection,
                                      soidBase,
                                      replaces,
                                      sessionId,
