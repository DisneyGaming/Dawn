 ...
 * A set authored in two bubbles (the Omega gate tunnel) has one point on each side, and the
 * caller's own position picks the side it is standing in.
 * @param setHash FNV-1 of the spawn-set name.
 * @param position World position to measure from.
 * @param point Receives the nearest point of that set.
 * @param distance Receives the distance to it, in world units.
 * @return True when at least one point carries the hash.
 */
[[nodiscard]] bool nearest_point_of_set(std::uint32_t setHash,
                                        const std::array<float, kPositionComponents>& position,
                                        Point& point,
                                        float& distance) noexcept;
