...
/** Registry id a member uses to ask the host to remove one local player. */
...
/** Complete body of a player-remove message. */
struct PlayerRemoveRequest {
    std::uint64_t sessionId{};
    /** Index in the member's local-player array. */
...
/**
 * Reads a player-remove message.
 * @param reader Reader positioned at the body.
 * @param output Receives the session and local-player index.
 * @return True when the complete body was present.
 */
