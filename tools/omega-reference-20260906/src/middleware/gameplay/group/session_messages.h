...
/**
 * Body of a host-handoff message.
 *
 * This is the only native session message that can arm the manager's pending member index. The
 * address must byte-match the target member's address in the manager table.
 */
struct HostHandoff {
    std::uint64_t sessionId{};
...
/** Body of the fixed 16-byte host-transition message. */
struct HostTransition {
    std::uint64_t sessionId{};
    /** Native codec clamps this field to the inclusive range 0 through 100 and writes seven bits. */
    std::uint32_t count{};
    /** Opaque 32-bit tail retained in raw memory order by the native codec. */
    std::uint32_t value{};
};

/**
 * Body of a host-reestablish message.
 *
 * The native registry declares 136 decoded bytes. Its fixed codec writes the two identities,
 * serializes the NetAddr selected by its method byte, then writes the two opaque identity blocks.
 */
struct HostReestablish {
    std::uint64_t sessionId{};
    std::uint64_t machineId{};
    std::array<std::byte, descriptor::kNetAddrSize> address{};
    std::array<std::byte, 16> identity128{};
    std::array<std::byte, 18> identity144{};
};

static_assert(offsetof(HostReestablish, identity128) == 0x66U);
static_assert(offsetof(HostReestablish, identity144) == 0x76U);
static_assert(sizeof(HostReestablish) == kHostReestablishSize);
...
/** Writes a host-handoff body exactly as the native kind-19 codec does. */
[[nodiscard]] bool write_host_handoff(encoding::bits::Writer& writer,
                                      const HostHandoff& body) noexcept;

/** Writes a host-transition body exactly as the native kind-21 codec does. */
[[nodiscard]] bool write_host_transition(encoding::bits::Writer& writer,
                                         const HostTransition& body) noexcept;

/** Writes a host-reestablish body exactly as the native kind-22 codec does. */
[[nodiscard]] bool write_host_reestablish(encoding::bits::Writer& writer,
                                          const HostReestablish& body) noexcept;

/** Reads a host-reestablish body exactly as the native kind-22 codec does. */
