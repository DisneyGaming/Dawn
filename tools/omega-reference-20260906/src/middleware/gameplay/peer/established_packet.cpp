...
/** Prefix of the compact contiguous-history form. */
...
/**
 * Reports whether the stored history is one contiguous received run behind the head.
 * False entries after that run are packets from before this connection, not gaps. A true entry
 * after the first false one is a real hole and needs the explicit form.
 */
[[nodiscard]] bool history_is_contiguous(
    const std::array<bool, kAckHistory>& received) noexcept {
    bool reachedUntrackedHistory = false;
    for (const bool entry : received) {
        if (!entry) {
            reachedUntrackedHistory = true;
        } else if (reachedUntrackedHistory) {
            return false;
        }
    }
    return true;
...
bool write_head_and_ack(bits::Writer& writer,
    ...
    if (history_is_contiguous(ack.received)) {
        // Captured client packets use this form whenever the history has no holes. The trailing
        // bit retires everything before the acknowledged head, so an ACK-only packet stays at
        // the client's canonical six-byte width instead of changing grammar after establishment.
        return writer.write(kStatusEmpty, kStatusPrefixWidth)
               && writer.write(1, kFlagWidth) && writer.write(ack.delay, kDelayWidth);
    }
    if (!writer.write(kStatusExplicit, kStatusPrefixWidth)) {
        return false;
    }
...
/** Writes a prefix of one reliable queue. */
bool write_queue(bits::Writer& writer,
                 const state::gameplay::OutboundQueue& queue,
                 std::size_t recordCount) noexcept {
    if (recordCount > queue.count) {
        return false;
    }
    if (recordCount == 0) {
