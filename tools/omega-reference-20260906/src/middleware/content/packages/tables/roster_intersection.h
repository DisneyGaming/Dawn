 ...
 * Navigation type 19 remains available because Crescent/Shadowkeep needs its registration and
 * other destinations legitimately declare their own type-19 groups. Authored partial layouts are
 * selected separately from scenario-root/package/bubble provenance, never by expanding this type
 * membership. The key limit below holds only for this filtered set. Feeding every placed object
 * instead overflows most destinations.
 */
inline constexpr std::array<std::uint16_t, 10> kRosterSlotTypes = {
    8, 13, 16, 17, 19, 21, 35, 37, 41, 67};
...
/**
 * Reports keys present in some observed slice sets but not all, together with the bubble mask
 * that can safely register each key through the roster delta's per-bubble sub-block.
 */
